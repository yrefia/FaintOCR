...
# 4.0.0
# 新增首次欢迎页、聚焦式新手教程遮罩、悬浮球菜单引导与任务清单进度。
# 4.0.0b
# 修复新手教程语言同步、遮罩点击阻塞、引导卡片按钮位置、菜单控件合并介绍与拖动跟随。
# 4.0.0c
# 从欢迎页开始显示遮罩，遮罩拦截背景点击，进度面板显示动画状态并承载跳过按钮。
# 4.0.0d
# 恢复菜单控件逐个介绍，修正遮罩点击后的层级，并优化进度完成动画。
# 4.0.1
# 新手教程继续扩展至设置、依赖文件、OCR 模式与首次识别结果。
# 遮罩行为回滚到 4.0.0b：透明输入，不拦截点击，也不主动抬高前景窗口。
# 4.0.1c
# 基于 4.0.1 遮罩实现修正欢迎页下拉框、菜单悬停引导、设置面板聚焦与教程误点击锁定。
# 4.0.1d
# 修复设置面板教程阶段的层级闪动，并在高亮设置项前自动滚动到目标控件。
# 4.0.1e
# 改进教程动作提示、上一步状态回退、匹配数据生成流程，并避免 OCR 模式切换自动推进。
# 4.0.1f
# 设置面板教程改为仅由引导卡片推进，修正最终 OCR 聚焦、操作提示布局与设置面板误关闭。
# 4.0.1g
# 重写教程进度映射，优化任务完成勾选视觉，并让一键生成介绍直接由下一步推进。
# 4.0.2
# 合并教程准备数据任务，点击执行 OCR 即完成教程，并收紧教程期间的悬浮球与设置面板交互锁。
# 4.0.2b
# 修复教程点击设置按钮阶段的悬浮球菜单悬停光效。
# 4.1.0
# 为 Windows 任务栏图标设置进程 AppUserModelID。
# 4.1.0b (+Deepseek)(+Gemini)
# "class FaintOCRApplication": "def show": self.ball.show()
# 后添加了```
#         icon = QtGui.QIcon(str(RUNTIME_ICON_FILE))
#         if not icon.isNull():
#             self.ball.setWindowIcon(icon)
#             # 通过微调标志强制 Windows 刷新任务栏缓存（无副作用）
#             flags = self.ball.windowFlags()
#             self.ball.setWindowFlags(flags)
# ```解决了未正确在任务栏显示自定义图标的问题。
# 4.1.0c
# 添加了两处图标设置错误的I18n文本
...
WINDOWS_APP_USER_MODEL_ID = "Lightscape.FaintOCR"
...
def set_windows_app_user_model_id(app_id=WINDOWS_APP_USER_MODEL_ID):
    ...
    try:
        ...
        logger.warning(I18n.get("log_app_user_model_id_failed", e))
...
class FloatingDebugLogBridge(QtCore.QObject):
    ...
    def install(self, slot):
        if self.sink_id is not None:
            return
        self.message_signal.connect(slot)
...
class I18n:
    ...
    TEXT = {
        "zh_CN": {
            ...
            "state_missing": "🔴 缺失: {0}",
            "state_stale": "🟠 本地文件可更新，可点击修复或继续使用旧版本",
            "state_missing_stale": "🔴🟠 文件缺失，且部分文件可更新",
            "state_error": "⚠️ 错误: {0}",
            ...
            "ui_status_connected": "FaintOCR 4.1.0 已连接 OCR 控制器",
            "ui_section_basic": "基础设置",
            "ui_reset_defaults": "恢复默认设置",
            ...
            "ui_match_data_ready": "匹配数据已为最新",
            "ui_match_data_loading": "匹配数据加载中",
            "ui_match_data_missing": "🔴 缺失，点击生成",
            "ui_match_data_stale": "🟠 可更新，点击修复",
            "ui_match_data_missing_stale": "🔴🟠 有缺失，有可更新，点击修复",
            "ui_dict_file_state_unknown": "远程仓库最后更新时间：-\n本地文件状态：未检查",
            "ui_dict_file_state": "远程仓库最后更新时间：{0}\n本地最早文件创建时间：{1}",
            "ui_dict_file_remote_unknown": "远程仓库最后更新时间：不可用\n本地最早文件创建时间：{0}",
            "ui_dict_local_none": "无",
            "ui_dict_memory_ready": "内存就绪",
            "ui_dict_memory_missing": "内存未满足",
            "ui_dict_memory_stale": "本地可用，有更新",
            ...
            "log_config_load_failed": "读取用户配置失败，使用默认配置: {0}",
            "log_config_save_failed": "保存用户配置失败: {0}",
            "log_gitlab_update_failed": "GitLab 仓库更新时间检查失败: {0} · {1}",
            "log_dict_cancel_requested": "字典任务收到取消指令: {0}",
            "log_dict_start": "开始字典任务: {0} Type={1} Mask={2}",
            "log_dict_cancelled": "字典任务在中途已取消",
            "log_dict_success": "字典任务执行成功",
            "log_dict_worker_error": "字典加载线程发生异常",
            "log_dict_merge_ok": "字典合并成功: {0} 条目",
            "log_dict_merge_failed": "字典合并失败: {0}",
            "log_dict_remove_failed": "删除字典文件失败: {0} · {1}",
            "log_dict_remove_stale_raw_failed": "删除过期原始字典文件失败: {0} · {1}",
            "log_dict_remove_stale_map_failed": "删除过期匹配字典文件失败: {0} · {1}",
            "log_dict_start_debug": "开始字典任务: {0} {1}→{2} Type={3} Mask={4}",
            "log_dict_finished_debug": "字典任务完成: {0} {1}→{2} Type={3} Mask={4} Missing={5} Stale={6}",
            "log_dict_skip_debug": "跳过字典任务: {0} {1}→{2} Type={3} Mask=0",
            "log_dict_remote_result": "远程仓库最后更新时间: {0} {1}",
            "log_dict_remote_unavailable": "远程仓库最后更新时间不可用: {0}",
            ...
            "log_atomic_write_failed": "原子写入失败（磁盘满或权限不足）: {0} · {1}",
            "err_ocr_engine_uninitialized": "OCR 引擎未初始化",
            "err_capture_exception": "截图异常: {0}",
            "err_image_data_missing": "图像数据缺失",
            "err_static_candidates_mismatch": "静态匹配候选与基础映射不一致",
            "err_json_object_expected": "{0} 内容不是 JSON 对象",
            "err_download_failed": "下载 {0} 失败: {1}",
            "err_download_no_data": "没有可保存的数据: {0}",
            "err_download_save_failed": "保存合并文件 {0} 失败",
            ...
            "log_icon_load_failed": "图标加载失败，请检查路径！",
            "log_app_user_model_id_failed": "设置 Windows AppUserModelID 失败: {0}",
            "log_cleanup": "FaintOCR 4.1.0 资源清理完成",
            ...
            "log_filter_on": "开",
            "log_filter_off": "关",
            "tutorial_welcome_title": "欢迎使用 FaintOCR",
            "tutorial_welcome_body": "先选择界面语言，然后用一个很短的新手教程熟悉悬浮球菜单。",
            "tutorial_language_label": "界面语言",
            "tutorial_start": "开始教程",
            "tutorial_skip": "跳过教程",
            "tutorial_progress_title": "新手任务",
            "tutorial_progress": "进度 {0}/{1}",
            "tutorial_task_welcome": "选择界面语言",
            "tutorial_task_expand_menu": "展开悬浮球菜单",
            "tutorial_task_learn_menu": "认识悬浮球菜单",
            "tutorial_task_open_settings": "打开设置面板",
            "tutorial_task_prepare_files": "准备语言与匹配数据",
            "tutorial_task_choose_ocr_mode": "选择 OCR 模式",
            "tutorial_task_first_ocr": "完成一次 OCR",
            "tutorial_step_expand_title": "展开悬浮球菜单",
            "tutorial_step_expand_body": "把鼠标移到悬浮球上，真实菜单会从旁边展开。展开后任务会自动完成。",
            "tutorial_step_menu_title": "认识悬浮球菜单",
            "tutorial_step_menu_body": "菜单上有四个核心控件。按下一步逐个查看它们的作用。",
            "tutorial_menu_subtitle_title": "字幕窗口",
            "tutorial_menu_subtitle_body": "显示或隐藏识别结果窗口，用来查看 OCR 返回文本。",
            "tutorial_menu_power_title": "OCR 按钮",
            "tutorial_menu_power_body": "在手动模式下点击一次执行 OCR；在自动或检测模式下用于启停。",
            "tutorial_menu_region_title": "识别区域",
            "tutorial_menu_region_body": "重新框选游戏字幕区域。区域选得准，识别会稳定很多。",
            "tutorial_menu_settings_title": "设置面板",
            "tutorial_menu_settings_body": "打开游戏、语言、匹配、OCR 模式和输出设置。",
            "tutorial_step_open_settings_title": "打开设置面板",
            "tutorial_step_open_settings_body": "点击悬浮球菜单里的设置按钮，打开 FaintOCR 设置面板。",
            "tutorial_step_prepare_files_title": "选择游戏语言并生成依赖文件",
            "tutorial_step_prepare_files_body": "在设置面板里确认游戏、源语言和目标语言。如果匹配数据缺失，点击“生成匹配数据”。",
            "tutorial_step_match_mode_title": "选择匹配模式",
            "tutorial_step_match_mode_body": "在匹配设置里选择一种匹配模式。建议新手选择“精确匹配”或“前缀匹配”，不要保持关闭。",
            "tutorial_step_generate_title": "生成或下载匹配数据",
            "tutorial_step_generate_body": "这里会说明“生成匹配数据”按钮的作用：当匹配数据缺失或过期时，它会下载源文件并生成依赖。确认后点击下一步。",
            "tutorial_step_ocr_mode_title": "选择 OCR 模式",
            "tutorial_step_ocr_mode_body": "在 OCR 运行区域选择模式。建议第一次使用“手动”，这样点击一次就识别一次。",
            "tutorial_step_first_ocr_title": "进行 OCR 并获得返回结果",
            "tutorial_step_first_ocr_body": "回到悬浮球，点击悬浮球本体执行一次 OCR。点击后会解除教学禁用并完成教程。",
            "tutorial_action_hover_ball": "将鼠标移动到悬浮球上以继续",
            "tutorial_action_hover_icon": "将鼠标移动到高亮图标上以继续",
            "tutorial_action_click_settings": "点击高亮的设置按钮以继续",
            "tutorial_action_choose_match": "选择“精确匹配”或“前缀匹配”以继续",
            "tutorial_action_click_generate": "点击高亮的生成匹配数据按钮并等待完成",
            "tutorial_action_click_ocr": "点击高亮的悬浮球以执行 OCR",
            "tutorial_next": "下一步",
            "tutorial_back": "上一步",
            "tutorial_finish": "完成",
        ...
        "en_US": {
            ...
            "state_stale": "🟠 Local files can be updated; click repair or keep using local files",
            "state_missing_stale": "🔴🟠 Files missing, and some local files can be updated",
            ...
            "ui_status_connected": "FaintOCR 4.1.0 connected to OCR controller",
            "ui_section_basic": "Basic Settings",
            "ui_reset_defaults": "Restore Defaults",
            ...
            "ui_match_data_ready": "Matching data is up to date",
            "ui_match_data_loading": "Matching data is loading",
            "ui_match_data_missing": "🔴 Missing; click to generate",
            "ui_match_data_stale": "🟠 Update available; click to repair",
            "ui_match_data_missing_stale": "🔴🟠 Missing and update available; click to repair",
            "ui_dict_file_state_unknown": "Remote repository last update: -\nLocal file status: not checked",
            "ui_dict_file_state": "Remote repository last update: {0}\nOldest local file creation time: {1}",
            "ui_dict_file_remote_unknown": "Remote repository last update: unavailable\nOldest local file creation time: {0}",
            "ui_dict_local_none": "none",
            "ui_dict_memory_ready": "Memory Ready",
            "ui_dict_memory_missing": "Memory Missing",
            "ui_dict_memory_stale": "Local usable, update available",
            ...
            "log_config_load_failed": "Failed to load user config; using defaults: {0}",
            "log_config_save_failed": "Failed to save user config: {0}",
            "log_gitlab_update_failed": "GitLab repository update check failed: {0} · {1}",
            ...
            "log_dict_success": "Dictionary task completed successfully",
            ...
            "log_dict_remove_failed": "Failed to remove dictionary file: {0} · {1}",
            "log_dict_remove_stale_raw_failed": "Failed to remove stale raw dictionary file: {0} · {1}",
            "log_dict_remove_stale_map_failed": "Failed to remove stale map dictionary file: {0} · {1}",
            "log_dict_start_debug": "Starting dictionary task: {0} {1}→{2} Type={3} Mask={4}",
            "log_dict_finished_debug": "Dictionary task finished: {0} {1}→{2} Type={3} Mask={4} Missing={5} Stale={6}",
            "log_dict_skip_debug": "Skipped dictionary task: {0} {1}→{2} Type={3} Mask=0",
            "log_dict_remote_result": "Remote repository last update: {0} {1}",
            "log_dict_remote_unavailable": "Remote repository update time unavailable: {0}",
            ...
            "log_atomic_write_failed": "Atomic write failed (disk full or insufficient permissions): {0} · {1}",
            "err_ocr_engine_uninitialized": "OCR engine is not initialized",
            "err_capture_exception": "Screen capture exception: {0}",
            "err_image_data_missing": "Image data is missing",
            "err_static_candidates_mismatch": "Static match candidates do not match the base mapping",
            "err_json_object_expected": "{0} content is not a JSON object",
            "err_download_failed": "Download {0} failed: {1}",
            "err_download_no_data": "No savable data: {0}",
            "err_download_save_failed": "Failed to save merged file {0}",
            ...
            "log_icon_load_failed": "Icon loading failed. Please check the path.",
            "log_app_user_model_id_failed": "Failed to set Windows AppUserModelID: {0}",
            "log_cleanup": "FaintOCR 4.1.0 resources cleaned up",
            ...
            "log_filter_on": "on",
            "log_filter_off": "off",
            "tutorial_welcome_title": "Welcome to FaintOCR",
            "tutorial_welcome_body": "Choose the interface language first, then take a short guided tour of the floating menu.",
            "tutorial_language_label": "Interface language",
            "tutorial_start": "Start Tutorial",
            "tutorial_skip": "Skip Tutorial",
            "tutorial_progress_title": "New User Tasks",
            "tutorial_progress": "Progress {0}/{1}",
            "tutorial_task_welcome": "Choose interface language",
            "tutorial_task_expand_menu": "Open the floating menu",
            "tutorial_task_learn_menu": "Learn the floating menu",
            "tutorial_task_open_settings": "Open settings",
            "tutorial_task_prepare_files": "Prepare languages and matching data",
            "tutorial_task_choose_ocr_mode": "Choose OCR mode",
            "tutorial_task_first_ocr": "Run OCR once",
            "tutorial_step_expand_title": "Open the floating menu",
            "tutorial_step_expand_body": "Move the mouse over the floating ball. The real menu will expand beside it and this task will complete automatically.",
            "tutorial_step_menu_title": "Learn the floating menu",
            "tutorial_step_menu_body": "The menu has four core controls. Use Next to review them one by one.",
            "tutorial_menu_subtitle_title": "Subtitle Window",
            "tutorial_menu_subtitle_body": "Show or hide the result window where OCR text appears.",
            "tutorial_menu_power_title": "OCR Button",
            "tutorial_menu_power_body": "In manual mode it runs OCR once; in auto or detect mode it starts or stops OCR.",
            "tutorial_menu_region_title": "Recognition Region",
            "tutorial_menu_region_body": "Select the game subtitle area again. A precise region makes OCR steadier.",
            "tutorial_menu_settings_title": "Settings Panel",
            "tutorial_menu_settings_body": "Open game, language, matching, OCR mode, and output settings.",
            "tutorial_step_open_settings_title": "Open settings",
            "tutorial_step_open_settings_body": "Click the settings button in the floating menu to open the FaintOCR settings panel.",
            "tutorial_step_prepare_files_title": "Choose game languages and prepare files",
            "tutorial_step_prepare_files_body": "Confirm the game, source language, and target language. If matching data is missing, click Generate Matching Data.",
            "tutorial_step_match_mode_title": "Choose matching mode",
            "tutorial_step_match_mode_body": "Choose a matching mode in Matching settings. Exact or Prefix matching is recommended; do not leave it off.",
            "tutorial_step_generate_title": "Generate or download matching data",
            "tutorial_step_generate_body": "This step explains what Generate Matching Data does: when matching data is missing or stale, it downloads source files and builds the required data. Click Next after confirming it.",
            "tutorial_step_ocr_mode_title": "Choose OCR mode",
            "tutorial_step_ocr_mode_body": "Choose a mode in OCR Operation. Manual mode is recommended for the first run.",
            "tutorial_step_first_ocr_title": "Run OCR and get a result",
            "tutorial_step_first_ocr_body": "Return to the floating ball and click the ball itself to run OCR once. The tutorial unlocks interactions and completes after the click.",
            "tutorial_action_hover_ball": "Move the mouse over the floating ball to continue",
            "tutorial_action_hover_icon": "Move the mouse over the highlighted icon to continue",
            "tutorial_action_click_settings": "Click the highlighted settings button to continue",
            "tutorial_action_choose_match": "Choose Exact or Prefix matching to continue",
            "tutorial_action_click_generate": "Click the highlighted Generate Matching Data button and wait",
            "tutorial_action_click_ocr": "Click the highlighted floating ball to run OCR",
            "tutorial_next": "Next",
            "tutorial_back": "Back",
...
class Config:
    ...
    USER_CONFIG_FILE = os.path.join(BASE_DIR, "UserConfig.json")
    ...
    WINDOW_DEFAULTS = {
        "floating_ball_anchor": None,
        "subtitle_geometry": None,
        "subtitle_controls_visible": True,
    }
    ...
    GAMES = {
        "Genshin Impact": {
            ...
            "gitlab_project": "Dimbreath/AnimeGameData",
            "urls": {
                "TextMapCHS.json": [
...
class UserConfigManager:
    ...
    @classmethod
    def _profile_key(cls, game_key, source_lang):
        return f"{game_key}|{source_lang}"

    @classmethod
    def _default_profiles(cls):
        return {
            cls._profile_key(game_key, source_lang): dict(profile)
    ...
    def default_config(cls):
        return {
            ...
            "ui": {
                "interface_language": Config.DEFAULTS["interface_language"],
            },
            "ocr": {
                "mode": Config.DEFAULTS["ocr_mode"],
                "auto_frequency_per_4s": Config.interval_to_auto_frequency(Config.OCR["DEFAULT_INTERVAL"]),
                "detect_frequency": Config.DEFAULTS["detect_frequency"],
                "detect_sensitivity": Config.DEFAULTS["detect_sensitivity"],
                "region": list(Config.OCR["DEFAULT_REGION"]),
            },
            "match": {
                "game": Config.DEFAULTS["game"],
                "source_lang": Config.DEFAULTS["source_lang"],
                "target_lang": Config.DEFAULTS["target_lang"],
                "match_mode": Config.DEFAULTS["match_mode"],
                "similarity_threshold": Config.DEFAULTS["similarity_threshold"],
            },
            "output": {
                "auto_copy": Config.DEFAULTS["auto_copy"],
                "auto_save": Config.DEFAULTS["auto_save"],
            },
            "subtitle": {
                "font_size": Config.DEFAULTS["subtitle_font_size"],
                "opacity": Config.DEFAULTS["subtitle_opacity"],
                "visible": Config.DEFAULTS["subtitle_visible"],
            },
            "windows": dict(Config.WINDOW_DEFAULTS),
            "tutorial": {
                "completed": False,
                "welcome_completed": False,
                "last_step": 0,
            },
            "personalization_profiles": cls._default_profiles(),
        }
    ...
    def load(cls):
        try:
            ...
            with open(Config.USER_CONFIG_FILE, "rb") as config_file:
                raw_config = orjson.loads(config_file.read())
            return cls.validate(raw_config)
        except Exception as exc:
            logger.warning(I18n.get("log_config_load_failed", exc))
            return cls.default_config()

    @classmethod
    def save(cls, config):
        try:
            validated = cls.validate(config)
            temp_file = f"{Config.USER_CONFIG_FILE}.tmp"
            with open(temp_file, "wb") as config_file:
                config_file.write(orjson.dumps(validated, option=orjson.OPT_INDENT_2))
            os.replace(temp_file, Config.USER_CONFIG_FILE)
        except Exception as exc:
            logger.error(I18n.get("log_config_save_failed", exc))
    ...
    def validate(cls, raw_config):
        ...
        raw_ocr = raw_config.get("ocr", {})
        raw_match = raw_config.get("match", {})
        raw_output = raw_config.get("output", {})
        raw_subtitle = raw_config.get("subtitle", {})
        raw_windows = raw_config.get("windows", {})
        raw_tutorial = raw_config.get("tutorial", {})

        if isinstance(raw_ui, dict):
            language = raw_ui.get("interface_language")
            if language in I18n.TEXT:
                config["ui"]["interface_language"] = language

        if isinstance(raw_ocr, dict):
            config["ocr"]["mode"] = cls._choice(raw_ocr.get("mode"), (0, 1, 2), config["ocr"]["mode"])
            config["ocr"]["auto_frequency_per_4s"] = cls._clamp_int(
                raw_ocr.get("auto_frequency_per_4s"),
                *Config.UI_RANGES["auto_frequency_per_4s"],
                config["ocr"]["auto_frequency_per_4s"],
            )
            config["ocr"]["detect_frequency"] = cls._clamp_int(
                raw_ocr.get("detect_frequency"),
                *Config.UI_RANGES["detect_frequency"],
                config["ocr"]["detect_frequency"],
            )
            config["ocr"]["detect_sensitivity"] = cls._clamp_int(
                raw_ocr.get("detect_sensitivity"),
                *Config.UI_RANGES["detect_sensitivity"],
                config["ocr"]["detect_sensitivity"],
            )
            region = cls._int_list(raw_ocr.get("region"), 4)
            if region and region[2] > 0 and region[3] > 0:
                config["ocr"]["region"] = region

        if isinstance(raw_match, dict):
            game = raw_match.get("game")
            if game in Config.GAMES:
                config["match"]["game"] = game
            source_lang = raw_match.get("source_lang")
            target_lang = raw_match.get("target_lang")
            if source_lang in Config.LANG_MAP:
                config["match"]["source_lang"] = source_lang
            if target_lang in Config.LANG_MAP:
                config["match"]["target_lang"] = target_lang
            if config["match"]["source_lang"] == config["match"]["target_lang"]:
                config["match"]["target_lang"] = Config.DEFAULTS["target_lang"]
                if config["match"]["source_lang"] == config["match"]["target_lang"]:
                    config["match"]["target_lang"] = "EN"
            config["match"]["match_mode"] = cls._choice(
                raw_match.get("match_mode"), (0, 1, 2), config["match"]["match_mode"]
            )
            config["match"]["similarity_threshold"] = cls._clamp_int(
                raw_match.get("similarity_threshold"),
                *Config.UI_RANGES["similarity_threshold"],
                config["match"]["similarity_threshold"],
            )

        if isinstance(raw_output, dict):
            config["output"]["auto_copy"] = cls._bool(raw_output.get("auto_copy"), config["output"]["auto_copy"])
            config["output"]["auto_save"] = cls._bool(raw_output.get("auto_save"), config["output"]["auto_save"])

        if isinstance(raw_subtitle, dict):
            config["subtitle"]["font_size"] = cls._clamp_int(
                raw_subtitle.get("font_size"),
                *Config.UI_RANGES["subtitle_font_size"],
                config["subtitle"]["font_size"],
            )
            config["subtitle"]["opacity"] = cls._clamp_float(
                raw_subtitle.get("opacity"),
                *Config.UI_RANGES["subtitle_opacity"],
                config["subtitle"]["opacity"],
            )
            config["subtitle"]["visible"] = cls._bool(raw_subtitle.get("visible"), config["subtitle"]["visible"])

        if isinstance(raw_windows, dict):
            ball_anchor = cls._int_list(raw_windows.get("floating_ball_anchor"), 2)
            if ball_anchor:
                config["windows"]["floating_ball_anchor"] = ball_anchor
            subtitle_geometry = cls._int_list(raw_windows.get("subtitle_geometry"), 4)
            if subtitle_geometry and subtitle_geometry[2] > 0 and subtitle_geometry[3] > 0:
                config["windows"]["subtitle_geometry"] = subtitle_geometry
            if "subtitle_controls_visible" in raw_windows:
                config["windows"]["subtitle_controls_visible"] = cls._bool(
                    raw_windows.get("subtitle_controls_visible"),
                    config["windows"]["subtitle_controls_visible"],
                )

        if isinstance(raw_tutorial, dict):
            config["tutorial"]["completed"] = cls._bool(
                raw_tutorial.get("completed"),
                config["tutorial"]["completed"],
            )
            config["tutorial"]["welcome_completed"] = cls._bool(
                raw_tutorial.get("welcome_completed"),
                config["tutorial"]["welcome_completed"],
            )
            config["tutorial"]["last_step"] = cls._clamp_int(
                raw_tutorial.get("last_step"),
                0,
                8,
                config["tutorial"]["last_step"],
            )

        config["personalization_profiles"] = cls._validated_profiles(
            raw_config.get("personalization_profiles", {})
        )
        return config

    @classmethod
    def _validated_profiles(cls, raw_profiles):
        profiles = cls._default_profiles()
        if not isinstance(raw_profiles, dict):
            return profiles
        for key, profile in raw_profiles.items():
            if not isinstance(key, str) or "|" not in key or not isinstance(profile, dict):
                continue
            game_key, source_lang = key.split("|", 1)
            if game_key not in Config.GAMES or source_lang not in Config.LANG_MAP:
                continue
            nickname = str(profile.get("player_nickname", "")).strip()[:64]
            gender = "male" if profile.get("protagonist_gender") == "male" else "female"
            profiles[cls._profile_key(game_key, source_lang)] = {
...
class GitLabRepositoryChecker:
    API_ROOT = "https://gitlab.com/api/v4/projects"

    @classmethod
    def fetch_last_activity_timestamp(cls, game_key):
        project = Config.GAMES.get(game_key, {}).get("gitlab_project")
        if not project:
            return None
        try:
            encoded_project = quote(project, safe="")
            url = f"{cls.API_ROOT}/{encoded_project}"
            response = requests.get(url, timeout=6)
            response.raise_for_status()
            last_activity_at = response.json().get("last_activity_at")
            ...
            logger.warning(I18n.get("log_gitlab_update_failed", game_key, exc))
            return None
...
class DictionaryWorker(QtCore.QThread):
    ...
    def __init__(self, game_key, source_lang, target_lang, process_type, target_mask, remote_updated_at=None):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            if self.remote_updated_at is None:
                self.remote_updated_at = GitLabRepositoryChecker.fetch_last_activity_timestamp(self.game_key)
                if self.remote_updated_at:
                    logger.info(
                        I18n.get(
                            "debug_dict",
                            I18n.get(
                                "log_dict_remote_result",
                                I18n.game_name(self.game_key),
                                datetime.fromtimestamp(self.remote_updated_at).strftime("%Y-%m-%d %H:%M:%S"),
                            ),
                        )
                    )
                else:
                    logger.info(
                        I18n.get(
                            "debug_dict",
                            I18n.get("log_dict_remote_unavailable", I18n.game_name(self.game_key)),
                        )
                    )
...
class TextMapLoader:
    ...
    @classmethod
    def file_earliest_time(cls, path):
        try:
    ...
    @classmethod
    def is_file_older_than_remote(cls, path, remote_updated_at):
        if not remote_updated_at or not os.path.exists(path):
            return False
        file_time = cls.file_earliest_time(path)
        return file_time is not None and file_time < remote_updated_at

    @classmethod
    def files_for_mask(cls, source_lang, target_lang, mask):
        files = []
        if mask & cls.RAW_MASK:
            files.extend([
                f"TextMap{source_lang}.json",
                f"TextMap{target_lang}.json",
            ])
        if mask & cls.MAP_MASK:
            files.extend([
                f"CanonicalMap-{source_lang}to{target_lang}.json",
                f"MatchStatic-{source_lang}to{target_lang}.json",
            ])
        return files

    @classmethod
    def oldest_local_file_time(cls, game_key, source_lang, target_lang, mask):
        times = []
        for file_name in cls.files_for_mask(source_lang, target_lang, mask):
            path = cls.get_file_path(game_key, file_name)
    ...
    @classmethod
    def delete_files_for_mask(cls, game_key, source_lang, target_lang, mask):
        for file_name in cls.files_for_mask(source_lang, target_lang, mask):
            path = cls.get_file_path(game_key, file_name)
            if os.path.exists(path):
                try:
                    ...
                    logger.warning(I18n.get("log_dict_remove_failed", path, exc))

    @classmethod
    def handle_task(cls, game_key, source_lang, target_lang, process_type, target_mask, remote_updated_at=None, callbacks=None):
        ...
        # 2. 调用真正的核心处理逻辑
        missing_mask, stale_mask = cls._execute_task(
            game_key, source_lang, target_lang, process_type, target_mask, remote_updated_at, working_cache, callbacks
        )
        ...
        oldest_local_ctime = cls.oldest_local_file_time(game_key, source_lang, target_lang, original_target_mask)
        # 4. 返回 缺失掩码 + 可更新掩码 + 本地最早文件创建时间 + 生产成果(数据)
        return missing_mask, stale_mask, oldest_local_ctime, text_map, static_candidates

    @classmethod
    def _execute_task(cls, game_key, source_lang, target_lang, process_type, target_mask, remote_updated_at, working_cache, callbacks):
        ...
        if target_mask & 1:
            ...
            for cache_key, file_name in [("src", src_raw_file), ("tgt", tgt_raw_file)]:
                ...
                if is_r_exist and cls.is_file_older_than_remote(file_path, remote_updated_at):
                    if process_type >= 2:
                        try:
                            os.remove(file_path)
                        except OSError as exc:
                            logger.warning(I18n.get("log_dict_remove_stale_raw_failed", file_path, exc))
                        is_r_exist = False
                    else:
                        raw_stale = True
            ...
            if missing_raw:
                missing_mask |= cls.RAW_MASK
            elif raw_stale:
                stale_mask |= cls.RAW_MASK
        ...
        if target_mask & 2:
            ...
            if (
                is_m_exist
                and (
                    cls.is_file_older_than_remote(map_path, remote_updated_at)
                    or cls.is_file_older_than_remote(static_path, remote_updated_at)
                )
            ):
                if process_type >= 2:
                    for stale_path in (map_path, static_path):
                        if os.path.exists(stale_path):
                            try:
                                os.remove(stale_path)
                            except OSError as exc:
                                logger.warning(I18n.get("log_dict_remove_stale_map_failed", stale_path, exc))
                    is_m_exist = False
                else:
                    map_stale = True
            ...
            if process_type >= 1 and is_m_exist:
                try:
                    ...
                    if not isinstance(static_candidates, list) or len(static_candidates) != expected_static_count:
                        raise ValueError(I18n.get("err_static_candidates_mismatch"))
            ...
            # C. 判定缺失
            if missing_map:
                missing_mask |= cls.MAP_MASK
            elif map_stale:
                stale_mask |= cls.MAP_MASK

        # 3. 结束
        # =========================================================
        return missing_mask, stale_mask
        ...
        for name, configured_urls in urls.items():
            ...
            for source_index, url in enumerate(source_urls, start=1):
                ...
                for attempt in range(1, 4):
                    try:
                        if cancel_cb and cancel_cb(): return False, I18n.get("err_cancelled")
                        ...
                        with requests.get(url, stream=True, timeout=15, verify=False) as r:
                            ...
                            for chunk in r.iter_content(8192):
                                if cancel_cb and cancel_cb():
                                    raise InterruptedError(I18n.get("err_cancelled"))
                        ...
                        if not isinstance(source_data, dict):
                            raise ValueError(I18n.get("err_json_object_expected", display_name))
                        ...
                        return False, I18n.get("err_cancelled")
                    except Exception as err:
                        last_err = err
                else:
                    return False, I18n.get("err_download_failed", display_name, last_err)

            if cancel_cb and cancel_cb(): return False, I18n.get("err_cancelled")
            if merged_data is None: return False, I18n.get("err_download_no_data", name)

            # ⚠️ 注意这里：如果你为了压榨内存，indent=True 会显著增加生成 JSON 字符串时的内存和磁盘体积
            if not cls._save_file_atomic(path, merged_data, indent=True):
                return False, I18n.get("err_download_save_failed", name)
    ...
    def _save_file_atomic(cls, final_path, data, indent=False):
        ...
        try:
            ...
            logger.critical(I18n.get("log_atomic_write_failed", final_path, e))
...
class DetectorWorker(QtCore.QObject):
    ...
    def _on_timeout(self):
        try:
            ...
            if score > 1.0:
                logger.debug(I18n.get("debug_detect_poll", score, pixel_ratio, count_ratio))
                self._timer.stop()
...
class OCRWorker(QtCore.QRunnable):
    ...
    def extract_shape_by_color(
        ...
        for contour in contours:
            ...
            if debug:
                logger.debug(I18n.get("log_crop_candidate", x, y, w, h_box, center_offset))
    ...
    @classmethod
    def crop_image_below_detected_box(cls, img_cv, game_key):
        """
        ...
        if not crop_rules.get("enabled", False):
            logger.debug(I18n.get("debug_crop_skipped", I18n.get("debug_crop_reason_disabled")))
            return img_cv, False
        ...
        if not rule or not rule.get("enabled", False):
            logger.debug(I18n.get("debug_crop_skipped", I18n.get("debug_crop_reason_no_rule")))
            return img_cv, False
        ...
        if not boxes:
            logger.debug(I18n.get("debug_crop_skipped", I18n.get("debug_crop_reason_not_found")))
            return img_cv, False
        ...
        if crop_y <= 0 or crop_y >= img_cv.shape[0] - min_remaining_height:
            logger.debug(I18n.get("debug_crop_skipped", I18n.get("debug_crop_reason_invalid")))
            return img_cv, False

        cropped_img = img_cv[crop_y:, :].copy()
        logger.debug(I18n.get("debug_crop_applied", crop_y, image_h, image_h - crop_y))
        if debug_enabled:
    ...
    def run(self):
        try:
            ...
            # [Log] 记录开始 (Debug级别)
            logger.info(I18n.get("log_ocr_worker_start"))
            # 支持在子线程中截图

            if img_cv is None:
                raise ValueError(I18n.get("err_image_data_missing"))
            ...
            if match_mode != 0:
                img_cv, crop_succeeded = self.crop_image_below_detected_box(img_cv, game_key)
            # ========================================================
    ...
    def run_rapidocr(
            self, engine, img, game_key, source_lang, match_mode, crop_succeeded=False):
        """
        调用 RapidOCR 引擎进行推理
        ...
        if not result:
            logger.debug(I18n.get("log_rapidocr_empty"))
            return None

        logger.debug(I18n.get("log_rapidocr_raw", result))
        ...
        if match_mode == 0 or not crop_succeeded:
            ...
            if not height_filter_enabled:
                logger.debug(I18n.get("debug_height_filter_disabled"))
        ...
        if not filtered_result:
            logger.debug(I18n.get("log_filtered_empty"))
            return ""
...
class OCRTaskManager(QtCore.QObject):
    ...
    def submit_task(self, task_data):
        ...
        if len(self.task_queue) >= self.max_queue_size:
            ...
            logger.warning(I18n.get("log_queue_drop", len(self.task_queue)))
        ...
        # OCR task submission is a business event; only detection polling stays at TRACE.
        submit_message = I18n.get("log_task_submit", len(self.task_queue), self.active_tasks)
        logger.info(submit_message)
        self._process_queue()
...
class OCRController(QtCore.QObject):
    ...
    dict_file_state_signal = QtCore.Signal(object, object, int, int)
    ...
    def __init__(self):
        ...
        self.remote_repo_updated_cache = {}
        self.dict_remote_updated_at = None
        self.dict_oldest_local_ctime = None
        self.dict_missing_mask = 0
        self.dict_stale_mask = 0
    ...
    def set_game(self, game_name):
        ...
        self.dict_remote_updated_at = self.remote_repo_updated_cache.get(game_name)
        self._apply_personalization_profile()
    ...
    def _emit_dict_debug(self, message):
        logger.info(I18n.get("debug_dict", message))
    ...
    def _dict_context_args(self):
        return (
            I18n.game_name(self.current_game),
            I18n.language_name(self.source_lang),
            I18n.language_name(self.target_lang),
        )
    ...
    def set_personalization(self, nickname, gender):
        ...
        for source_lang in Config.PERSONALIZATION_DEFAULTS.get(self.current_game, {}):
            key = (self.current_game, source_lang)
            profile = self.personalization_profiles.get(key)
            if profile is None:
                profile = Config.personalization_default(*key)
            else:
                profile = dict(profile)
            profile["player_nickname"] = self.player_nickname
            if source_lang == self.source_lang:
                profile["protagonist_gender"] = self.protagonist_gender
            self.personalization_profiles[key] = profile
        self.text_matcher.set_profile(self.player_nickname, self.protagonist_gender)
    ...
    def request_ocr(self, region, task_source="manual"):
        ...
        if self.ocr_engine_std is None or self.ocr_engine_light is None:
            message = I18n.get("err_ocr_engine_uninitialized")
            logger.error(message)
            self.ocr_result_signal.emit(False, message, "", False, task_source)
            return False
        ...
        try:
            ...
            self.ocr_result_signal.emit(False, I18n.get("err_capture_exception", e), "", False, task_source)
            return False
        # ===================================================
    ...
    def _on_detection_triggered(self, score, rgb_image, session_id, change_detected_at):
        ...
        logger.info(I18n.get("log_detect_trigger", score))
    ...
    def _finish_detect_task(self, session_id, reason):
        ...
        logger.info(I18n.get("log_detect_task_done", reason, self.detect_pending_tasks))
    ...
    def _on_ocr_task_completed(self, is_success, main_text, status_info, allow_auto_output, task_source,
        ...
        if is_success:
            ...
            logger.info(I18n.get("log_ocr_complete", log_text, status_info) + "\n")
        else:
            logger.error(I18n.get("log_task_failed", main_text) + "\n")
    ...
    def _dispatch_worker(self, process_type):
        ...
        if process_type >= 2:
            target_mask = self.dict_missing_mask | self.dict_stale_mask
            if target_mask == 0:
                target_mask = mem_missing
        else:
            target_mask = mem_missing

        # 已满足目标时不应进入没有 Worker 回调的 LOADING 状态。
        if target_mask == 0:
            self._emit_dict_debug(
                I18n.get(
                    "log_dict_skip_debug",
                    *self._dict_context_args(),
                    process_type
                )
            )
            if process_type > 0:
        ...
        # 4. 统一按需实例化并启动 Worker
        if target_mask > 0:
            remote_updated_at = self.remote_repo_updated_cache.get(self.current_game)
            self.dict_remote_updated_at = remote_updated_at
            self.dict_file_state_signal.emit(
                self.dict_remote_updated_at, self.dict_oldest_local_ctime,
                self.dict_missing_mask, self.dict_stale_mask
            )
            self._emit_dict_debug(
                I18n.get(
                    "log_dict_start_debug",
                    *self._dict_context_args(),
                    process_type,
                    target_mask
                )
            ...
            self.dict_worker = DictionaryWorker(
                self.current_game, self.source_lang, self.target_lang, process_type, target_mask, remote_updated_at
            )
    ...
    def run_smart_fix(self):
        ...
        required_mask = self.MODE_REQUIREMENTS.get(self.match_mode, 0)
        mem_missing = required_mask & ~self.current_data_mask
        if mem_missing == 0 and (self.dict_missing_mask | self.dict_stale_mask) == 0:
            self.auto_load_timer.stop()
            self.ui_lock_signal.emit(False)
            self._refresh_ui_state()
            return
    ...
    def _on_worker_finished(self, handle_game, handle_source_lang, handle_target_lang, process_type, target_mask):
        ...
        remote_updated_at = getattr(self.dict_worker, "remote_updated_at", None)
        self.remote_repo_updated_cache[handle_game] = remote_updated_at
        self.dict_remote_updated_at = remote_updated_at

        missing_mask, stale_mask, oldest_local_ctime, text_map, static_candidates = self.dict_worker.result
        self.dict_missing_mask = missing_mask
        self.dict_stale_mask = stale_mask
        self.dict_oldest_local_ctime = oldest_local_ctime
        self._emit_dict_debug(
            I18n.get(
                "log_dict_finished_debug",
                *self._dict_context_args(),
                process_type,
                target_mask,
                missing_mask,
        ...
        if process_type != 0:
            ...
            success_mask = target_mask & ~missing_mask
            self.current_data_mask |= success_mask

        # 2. 将 Worker 的质检结果抛给 UI 刷新器进行最终裁定
        self._refresh_ui_state(worker_check_result=(missing_mask, stale_mask))

    def _on_worker_error(self, success, msg):
        self.ui_lock_signal.emit(False)
        if not success:
            # ✅ 修改：直接把错误信息放进列表里，投递给主状态机！
            logger.error(I18n.get("debug_error", msg))
    ...
    def _refresh_ui_state(self, worker_check_result=None):
        ...
        if worker_check_result is not None:
            self.dict_missing_mask, self.dict_stale_mask = worker_check_result

        # 场景 1: 内存已经满足，万事大吉
        if mem_missing == 0:
            if self.dict_missing_mask and self.dict_stale_mask:
                self.ui_state_signal.emit("MISSING_STALE", [self.dict_missing_mask, self.dict_stale_mask], self.current_data_mask, required_mask)
                return
            if self.dict_missing_mask:
                self.ui_state_signal.emit("MISSING", [self.dict_missing_mask], self.current_data_mask, required_mask)
                return
            if self.dict_stale_mask:
                self.ui_state_signal.emit("STALE", [], self.current_data_mask, required_mask)
                return
        ...
        if self.dict_missing_mask and self.dict_stale_mask:
            self.ui_state_signal.emit("MISSING_STALE", [self.dict_missing_mask, self.dict_stale_mask], self.current_data_mask, required_mask)
        elif self.dict_missing_mask:
            self.ui_state_signal.emit("MISSING", [self.dict_missing_mask], self.current_data_mask, required_mask)
        else:
            state = "STALE" if self.dict_stale_mask else "PENDING"
            self.ui_state_signal.emit(state, [], self.current_data_mask, required_mask)
            self.auto_load_timer.start(1200)
...
class FloatingBall(SoftWidget):
    ...
    def __init__(self, parent=None, floating=False):
        ...
        self.tutorial_keep_expanded = False
        self.tutorial_disable_menu_clicks = False
        self.tutorial_disable_all_clicks = False
        self.tutorial_allowed_action = ""
        self.tutorial_disable_hover_expand = False
    ...
    def preferred_menu_direction(self, anchor):
        area = self.screen().availableGeometry()
        ball_width = int(self.ball_rect().width())
        right_space = area.right() - (anchor.x() + ball_width)
        left_space = anchor.x() - area.left()
        return "right" if right_space >= self.MENU_REQUIRED_SPACE or right_space >= left_space else "left"

    def choose_menu_direction(self):
        self.set_menu_direction(self.preferred_menu_direction(self.ball_anchor_global()))
    ...
    def tutorial_target_rect(self, name):
        if name == "ball":
            return QRect(
                self.mapToGlobal(self.ball_rect().topLeft().toPoint()),
                self.ball_rect().size().toSize(),
            )
        if name == "menu":
            menu = self.menu_rect()
            return QRect(
                self.mapToGlobal(menu.topLeft().toPoint()),
                menu.size().toSize(),
            )
        for button_name, center in self.button_centers():
            if button_name == name:
                global_center = self.mapToGlobal(center.toPoint())
                return QRect(global_center.x() - 28, global_center.y() - 28, 56, 56)
        return QRect()
    ...
    def clear_tutorial_interaction(self):
        self.tutorial_keep_expanded = False
        self.tutorial_disable_menu_clicks = False
        self.tutorial_disable_all_clicks = False
        self.tutorial_allowed_action = ""
    ...
    def collapse(self):
        if self.tutorial_keep_expanded:
            return
        if not self._dragging:
    ...
    def mouseMoveEvent(self, event):
        if self.tutorial_disable_all_clicks:
            if self.tutorial_allowed_action == "expand":
                if self.ball_rect().adjusted(-8, -8, 8, 8).contains(event.position()):
                    self.expand()
                event.accept()
                return
            if self.tutorial_allowed_action in ("menu_hover", "settings"):
                self.hovered = ""
                for name, center in self.button_centers():
        ...
        if self.drag_to(event):
            self.collapse_timer.stop()
            if not self.tutorial_keep_expanded:
                self.set_expanded(False)
            self.hovered = ""
            event.accept()
            return

        if not self.tutorial_disable_hover_expand and self.interactive_region_contains(event.position()):
            self.expand()
        elif not self.tutorial_disable_hover_expand:
    ...
    def mousePressEvent(self, event):
        if self.tutorial_disable_all_clicks:
            if (
                self.tutorial_allowed_action == "settings"
                and event.button() == Qt.LeftButton
            ):
                for name, center in self.button_centers():
                    if name == "settings" and (event.position() - center).manhattanLength() < 30:
                        self.tutorial_keep_expanded = False
                        self.tutorial_allowed_action = ""
                        self.set_expanded(False)
            ...
            event.accept()
            return
        if self.tutorial_disable_menu_clicks:
            event.accept()
            return
        if event.button() == Qt.LeftButton:
    ...
    def mouseReleaseEvent(self, event):
        if self.tutorial_disable_all_clicks:
            self.finish_drag()
            if (
                self.tutorial_allowed_action == "ocr"
                and event.button() == Qt.LeftButton
                and self.ball_rect().contains(event.position())
            ):
                self.clear_tutorial_interaction()
                self.ocr_enabled = True if self.ocr_momentary else not self.ocr_enabled
                self.ocrChanged.emit(self.ocr_enabled)
                self.update()
                event.accept()
    ...
    def contextMenuEvent(self, event):
        if self.tutorial_disable_all_clicks or self.tutorial_disable_menu_clicks:
            event.accept()
            return
...
class TutorialWelcomeWindow(QtWidgets.QWidget):
    ...
    def __init__(self, initial_language):
        ...
        self.body_label.setObjectName("welcomeBody")
        self.body_label.setWordWrap(True)
        card_layout.addWidget(self.body_label)

        self.language_label = QtWidgets.QLabel()
        self.language_label.setObjectName("welcomeField")
        ...
        self.skip_button.clicked.connect(lambda: self.skipRequested.emit(self.current_language()))
        self.start_button.clicked.connect(lambda: self.startRequested.emit(self.current_language()))
    ...
    def current_language(self):
        return self.language_combo.currentData() or "zh_CN"
    ...
    def retranslate(self):
        self.title_label.setText(I18n.get("tutorial_welcome_title"))
        self.body_label.setText(I18n.get("tutorial_welcome_body"))
        self.language_label.setText(I18n.get("tutorial_language_label"))
        self.skip_button.setText(I18n.get("tutorial_skip"))
        self.start_button.setText(I18n.get("tutorial_start"))
...
class TutorialFocusOverlay(QtWidgets.QWidget):
    targetHovered = Signal()
    ...
    def set_focus(self, target_rect, title, body):
        self._target_rect = QRect(target_rect) if target_rect else QRect()
        self._title = title
        self._body = body
        self.refresh_screen_geometry()
        self.update()
    ...
    def paintEvent(self, _event):
        p = QPainter(self)
        p.setRenderHint(QPainter.Antialiasing)
        p.fillRect(self.rect(), color("#050711", 150))
        ...
        local_rect = QRectF(
            self._target_rect.translated(-origin.x(), -origin.y())
        ).adjusted(-10, -10, 10, 10)
        highlight = rounded_path(local_rect, 18)

        p.setCompositionMode(QPainter.CompositionMode_Clear)
        p.fillPath(highlight, QColor(0, 0, 0, 0))
        p.setCompositionMode(QPainter.CompositionMode_SourceOver)

        p.setPen(QPen(color("#a198ff", 230), 2.2))
        p.setBrush(Qt.NoBrush)
        p.drawPath(highlight)
        p.setPen(QPen(color("#ffffff", 70), 1.0))
        p.drawPath(rounded_path(local_rect.adjusted(-5, -5, 5, 5), 22))

    def mouseMoveEvent(self, event):
        if self._target_rect.isValid() and self._target_rect.contains(event.globalPosition().toPoint()):
    ...
    def draw_hint_card(self, painter, target_rect):
        card_width = 360
        card_height = 126
        margin = 24
        if target_rect.right() + 18 + card_width <= self.width() - margin:
            x = target_rect.right() + 18
        elif target_rect.left() - 18 - card_width >= margin:
            x = target_rect.left() - 18 - card_width
        else:
            x = min(max(margin, target_rect.center().x() - card_width / 2), self.width() - card_width - margin)
        y = min(max(margin, target_rect.top()), self.height() - card_height - margin)
        card = QRectF(x, y, card_width, card_height)

        painter.setPen(Qt.NoPen)
        painter.setBrush(color("#171925", 244))
        painter.drawRoundedRect(card, 16, 16)
        painter.setPen(QPen(color("#ffffff", 35), 1))
        painter.drawRoundedRect(card.adjusted(0.5, 0.5, -0.5, -0.5), 16, 16)

        title_font = QFont("Microsoft YaHei UI", 11)
        title_font.setBold(True)
        painter.setFont(title_font)
        painter.setPen(color("#f7f7fc"))
        painter.drawText(card.adjusted(18, 15, -18, -78), Qt.AlignLeft | Qt.AlignVCenter, self._title)

        body_font = QFont("Microsoft YaHei UI", 9)
        painter.setFont(body_font)
        painter.setPen(color("#b9becf"))
        painter.drawText(card.adjusted(18, 48, -18, -14), Qt.AlignLeft | Qt.TextWordWrap, self._body)


class TutorialGuideCard(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.action_label.setObjectName("guideAction")
        ...
        actions.addWidget(self.action_label, 1)
        actions.addStretch()
        ...
        self.setStyleSheet("""
            ...
            QLabel#guideAction {
                color: #f3f0ff;
                background: rgba(112, 105, 247, 42);
                border: 1px solid rgba(149, 142, 255, 90);
                border-radius: 8px;
                padding: 7px 9px;
                font-size: 9pt;
                font-weight: 600;
    ...
    def set_content(self, title, body, *, back_enabled, next_enabled, finish=False, action_hint=""):
        self.title_label.setText(title)
        self.body_label.setText(body)
        self.action_label.setText(action_hint)
        self.action_label.setVisible(bool(action_hint))
        self.back_button.setText(I18n.get("tutorial_back"))
        self.next_button.setText(I18n.get("tutorial_finish" if finish else "tutorial_next"))
        self.back_button.setEnabled(back_enabled)
        self.next_button.setEnabled(next_enabled)
        self.next_button.setVisible(next_enabled)
        self.adjustSize()

    def position_near(self, target_rect):
        screen = QApplication.primaryScreen().availableGeometry()
        margin = 24
        gap = 18
        width = self.width()
        height = self.height()
        target = QRect(target_rect) if target_rect else QRect()

        if target.isValid() and target.right() + gap + width <= screen.right() - margin:
            x = target.right() + gap
        elif target.isValid() and target.left() - gap - width >= screen.left() + margin:
            x = target.left() - gap - width
        else:
            center_x = target.center().x() if target.isValid() else screen.center().x()
            x = center_x - width // 2
        x = max(screen.left() + margin, min(x, screen.right() - width - margin))

        center_y = target.center().y() if target.isValid() else screen.center().y()
        y = center_y - height // 2
        y = max(screen.top() + margin, min(y, screen.bottom() - height - margin))
        self.move(x, y)


class TutorialTaskItem(QtWidgets.QWidget):
    ...
    def set_task_state(self, text, completed=False, active=False):
        became_completed = completed and not self._completed
        self._text = text
        self._active = active
        ...
        if became_completed:
            ...
            self.set_check_progress(1.0)
        else:
            self.animation.stop()
            self.set_check_progress(0.0)
        self.updateGeometry()
        self.update()
    ...
    def paintEvent(self, _event):
        p = QPainter(self)
        p.setRenderHint(QPainter.Antialiasing)
        center = QPointF(13, self.height() / 2)
        radius = 8
        row_rect = QRectF(0, 2, self.width(), self.height() - 4)

        if self._completed:
            p.setPen(Qt.NoPen)
            p.setBrush(color("#35d889", int(22 + 28 * self._check_progress)))
            p.drawRoundedRect(row_rect, 8, 8)
            p.setBrush(color("#35d889", int(58 + 52 * self._check_progress)))
            p.drawRoundedRect(QRectF(0, 6, 4, self.height() - 12), 2, 2)
        elif self._active:
            p.setPen(QPen(color("#8f88ff", 92), 1))
            p.setBrush(color("#8078ff", 22))
            p.drawRoundedRect(row_rect.adjusted(0.5, 0.5, -0.5, -0.5), 8, 8)

        if self._completed:
            pulse_radius = radius + 5 * (1.0 - self._check_progress)
            p.setPen(Qt.NoPen)
            p.setBrush(color("#35d889", int(65 * (1.0 - self._check_progress))))
            p.drawEllipse(center, pulse_radius, pulse_radius)
            p.setPen(QPen(color("#7cf1ad", int(130 + 80 * self._check_progress)), 1.4))
            p.setBrush(color("#24c978", int(185 + 60 * self._check_progress)))
            p.drawEllipse(center, radius, radius)
            p.setPen(QPen(color("#ffffff"), 2.2, Qt.SolidLine, Qt.RoundCap, Qt.RoundJoin))
            first = QPointF(center.x() - 4.2, center.y() - 0.2)
            middle = QPointF(center.x() - 1.0, center.y() + 3.4)
            last = QPointF(center.x() + 5.2, center.y() - 4.2)
            if self._check_progress <= 0.5:
                t = self._check_progress / 0.5
                end = QPointF(
                    first.x() + (middle.x() - first.x()) * t,
                    first.y() + (middle.y() - first.y()) * t,
                )
                p.drawLine(first, end)
            else:
                p.drawLine(first, middle)
                t = (self._check_progress - 0.5) / 0.5
                end = QPointF(
                    middle.x() + (last.x() - middle.x()) * t,
                    middle.y() + (last.y() - middle.y()) * t,
                )
                p.drawLine(middle, end)
        else:
            p.setPen(QPen(color("#ffffff", 210), 1.7))
            p.setBrush(Qt.NoBrush)
            p.drawEllipse(center, radius, radius)
        ...
        if self._completed:
            ...
            text_color = "#aeb3c7"
        p.setPen(color(text_color))
        p.drawText(QRectF(32, 0, self.width() - 34, self.height()), Qt.AlignVCenter | Qt.TextWordWrap, self._text)


class TutorialTaskPanel(QtWidgets.QWidget):
    nextRequested = Signal()
    skipRequested = Signal()

    TASK_KEYS = (
        "tutorial_task_welcome",
        "tutorial_task_expand_menu",
        "tutorial_task_learn_menu",
        "tutorial_task_open_settings",
        "tutorial_task_prepare_files",
        "tutorial_task_choose_ocr_mode",
        "tutorial_task_first_ocr",
    )
    ...
    def __init__(self):
        ...
        self.header_label = QtWidgets.QLabel(I18n.get("tutorial_progress_title"))
        self.header_label.setObjectName("taskHeader")
        layout.addWidget(self.header_label)

        self.progress_label = QtWidgets.QLabel()
        self.progress_label.setObjectName("taskProgress")
        layout.addWidget(self.progress_label)

        self.progress_bar = QtWidgets.QProgressBar()
        self.progress_bar.setRange(0, len(self.TASK_KEYS))
        self.progress_bar.setTextVisible(False)
        self.progress_bar.setFixedHeight(6)
        layout.addWidget(self.progress_bar)

        self.task_items = []
        for _key in self.TASK_KEYS:
            item = TutorialTaskItem()
            self.task_items.append(item)
            layout.addWidget(item)

        actions = QtWidgets.QHBoxLayout()
        self.skip_button = QtWidgets.QPushButton(I18n.get("tutorial_skip"))
    ...
    def set_state(self, completed_count, active_index, title, body, next_enabled=True, finish=False):
        total = len(self.TASK_KEYS)
        completed_count = max(0, min(int(completed_count), total))
        active_index = None if active_index is None else max(0, min(int(active_index), total - 1))
        self.header_label.setText(I18n.get("tutorial_progress_title"))
        self.progress_label.setText(I18n.get("tutorial_progress", completed_count, total))
        self.progress_bar.setValue(completed_count)
        for index, (item, key) in enumerate(zip(self.task_items, self.TASK_KEYS)):
            item.set_task_state(
                I18n.get(key),
                completed=index < completed_count and index != active_index,
...
class TutorialGuide(QtCore.QObject):
    finished = Signal()
    skipped = Signal()
    ...
    TASK_EXPAND_MENU = 1
    TASK_LEARN_MENU = 2
    TASK_OPEN_SETTINGS = 3
    TASK_PREPARE_FILES = 4
    TASK_OCR_MODE = 5
    TASK_FIRST_OCR = 6

    SETTINGS_TARGETS = {"game_language", "match_mode", "generate", "ocr_mode", "panel"}

    MENU_ITEMS = (
        ("subtitle", "tutorial_menu_subtitle_title", "tutorial_menu_subtitle_body"),
        ("power", "tutorial_menu_power_title", "tutorial_menu_power_body"),
        ("region", "tutorial_menu_region_title", "tutorial_menu_region_body"),
        ("settings", "tutorial_menu_settings_title", "tutorial_menu_settings_body"),
    )
    ...
    def __init__(self, app, overlay=None):
        ...
        self.overlay = overlay or TutorialFocusOverlay()
        self._owns_overlay = overlay is None
        self.panel = TutorialTaskPanel()
        self.card = TutorialGuideCard()
        self.stage = "expand"
        ...
        self.current_title = ""
        self.current_body = ""
        self._hover_confirm_target = None
        self._hover_confirm_started_at = 0.0
        self._last_focus_rect = QRect()
        self._last_focus_target = ""
        self._last_focus_title = ""
        self._last_focus_body = ""
        self._raised_focus_target = ""
        self._raised_card_target = ""
        self._step_generation = 0
        self._disposed = False
        self.card.backRequested.connect(self.back_step)
        self.card.nextRequested.connect(self.next_step)
        self.panel.skipRequested.connect(self.skip)
        self.ball.menuExpanded.connect(self.handle_menu_expanded)
        self.overlay.targetHovered.connect(self.handle_target_hovered)
        self.ball.ballMoved.connect(self.refresh_current_focus)
        self.ball.settingsRequested.connect(self.handle_settings_requested)
        self.settings.gameChanged.connect(self.handle_prepare_files_action)
        self.settings.languagesChanged.connect(self.handle_prepare_files_action)
        self.settings.smartFixRequested.connect(self.handle_prepare_files_action)
        self.ball.ocrChanged.connect(self.handle_tutorial_ocr_triggered)
        self.follow_timer = QTimer(self)
        self.follow_timer.setInterval(16)
        self.follow_timer.timeout.connect(self.on_follow_tick)

    def set_tutorial_progress(self, completed_count, active_index, title, body, next_enabled=True, finish=False):
        total = len(TutorialTaskPanel.TASK_KEYS)
        self.completed_count = max(0, min(int(completed_count), total))
        self.panel.set_state(self.completed_count, active_index, title, body, next_enabled=next_enabled, finish=finish)

    def configure_interaction(self, mode):
        self.ball.tutorial_disable_all_clicks = True
        self.ball.tutorial_disable_menu_clicks = True
        self.ball.tutorial_allowed_action = ""
        self.ball.tutorial_disable_hover_expand = True
        self.ball.tutorial_keep_expanded = False
        self.settings.tutorial_prevent_close = False
        self.settings.tutorial_prevent_drag = False
        if mode == "expand":
            self.ball.tutorial_allowed_action = "expand"
            self.ball.tutorial_disable_hover_expand = False
        elif mode == "menu":
            self.ball.tutorial_allowed_action = "menu_hover"
            self.ball.tutorial_keep_expanded = True
        elif mode == "open_settings":
            self.ball.tutorial_allowed_action = "settings"
            self.ball.tutorial_keep_expanded = True
        elif mode == "settings":
            self.settings.tutorial_prevent_close = True
            self.settings.tutorial_prevent_drag = True
        elif mode == "ocr":
            self.ball.tutorial_allowed_action = "ocr"
        elif mode == "free":
            self.ball.clear_tutorial_interaction()
    ...
    def run_if_current(self, token, callback):
        if self._disposed or token != self._step_generation:
            return
        callback()

    def schedule_current(self, delay_ms, callback):
        token = self._step_generation
        QTimer.singleShot(delay_ms, lambda: self.run_if_current(token, callback))

    def start(self):
        self.configure_interaction("locked")
        self.overlay.show()
        self.overlay.raise_()
        self.panel.show()
        self.card.show()
        self.follow_timer.start()
        self.show_expand_step()
        if self.ball.expanded:
            QTimer.singleShot(80, self.handle_menu_expanded)

    def show_expand_step(self):
        self.begin_step("expand")
        self.configure_interaction("expand")
        self.ball.collapse()
        title = I18n.get("tutorial_step_expand_title")
        body = I18n.get("tutorial_step_expand_body")
        self.set_tutorial_progress(self.TASK_EXPAND_MENU, self.TASK_EXPAND_MENU, title, body, next_enabled=False)
        self.card.set_content(
            title,
            body,
            back_enabled=False,
            next_enabled=False,
            action_hint=I18n.get("tutorial_action_hover_ball"),
        )
        self.set_current_focus("ball", title, body)

    def handle_menu_expanded(self):
        if self._disposed or self.stage != "expand":
            return
        self.completed_count = self.TASK_LEARN_MENU
        self.begin_step("menu")
        self.menu_index = 0
        self.configure_interaction("menu")
        self.ball.collapse_timer.stop()
        self.schedule_current(300, self.show_current_menu_item)

    def handle_target_hovered(self):
        if self._disposed:
            return
        if self.stage == "expand":
            ...
            return
        if self.stage == "menu":
            target_name = self.MENU_ITEMS[self.menu_index][0]
            now = time.monotonic()
            if self._hover_confirm_target != target_name:
                self._hover_confirm_target = target_name
                self._hover_confirm_started_at = now
                return
            if now - self._hover_confirm_started_at >= 0.28:
                self.next_step()

    def show_current_menu_item(self):
        if self._disposed or self.stage != "menu":
            return
        self.configure_interaction("menu")
        self.ball.collapse_timer.stop()
        if not self.ball.expanded:
            self.ball.set_expanded(True)
            self.schedule_current(300, self.show_current_menu_item)
            return
        if not self.ball.button_centers():
            self.schedule_current(50, self.show_current_menu_item)
            return

        target_name, title_key, body_key = self.MENU_ITEMS[self.menu_index]
        title = I18n.get(title_key)
        body = I18n.get(body_key)
        self.set_tutorial_progress(self.TASK_LEARN_MENU, self.TASK_LEARN_MENU, title, body, next_enabled=False)
        self.card.set_content(
            title,
            body,
            back_enabled=True,
            next_enabled=False,
            action_hint=I18n.get("tutorial_action_hover_icon"),
        )
        self._hover_confirm_target = None
        self._hover_confirm_started_at = 0.0
        self.set_current_focus(target_name, title, body)

    def ensure_settings_panel_open(self):
        if self.settings.isVisible():
            return
        self.settings.show_near(self.ball)
        self.schedule_current(280, self.refresh_current_focus)

    def show_open_settings_step(self):
        self.begin_step("open_settings")
        self.configure_interaction("open_settings")
        self.ball.collapse_timer.stop()
        if self.settings.isVisible():
            self.settings.tutorial_prevent_close = False
            self.settings.hide_animated(force=True)
        if not self.ball.expanded:
            self.ball.set_expanded(True)
            self.schedule_current(220, self.show_open_settings_step)
            return
        title = I18n.get("tutorial_step_open_settings_title")
        body = I18n.get("tutorial_step_open_settings_body")
        self.set_tutorial_progress(self.TASK_OPEN_SETTINGS, self.TASK_OPEN_SETTINGS, title, body, next_enabled=False)
        self.card.set_content(
            title,
            body,
            back_enabled=True,
            next_enabled=False,
            action_hint=I18n.get("tutorial_action_click_settings"),
        )
        self.set_current_focus("settings", title, body)

    def handle_settings_requested(self):
        if self._disposed or self.stage != "open_settings":
            return
        self.configure_interaction("settings")
        self.completed_count = self.TASK_PREPARE_FILES
        self.schedule_current(360, self.show_prepare_files_step)

    def show_prepare_files_step(self):
        if self._disposed:
            return
        self.begin_step("prepare_files")
        self.configure_interaction("settings")
        self.ensure_settings_panel_open()
        title = I18n.get("tutorial_step_prepare_files_title")
        body = I18n.get("tutorial_step_prepare_files_body")
        self.set_tutorial_progress(self.TASK_PREPARE_FILES, self.TASK_PREPARE_FILES, title, body, next_enabled=True)
        self.card.set_content(title, body, back_enabled=True, next_enabled=True)
        self.set_current_focus("game_language", title, body)

    def handle_prepare_files_action(self, *args):
        if self._disposed or self.stage != "prepare_files":
            return
        self.refresh_current_focus()

    def show_match_mode_step(self):
        if self._disposed:
            return
        self.begin_step("match_mode")
        self.configure_interaction("settings")
        self.ensure_settings_panel_open()
        title = I18n.get("tutorial_step_match_mode_title")
        body = I18n.get("tutorial_step_match_mode_body")
        self.set_tutorial_progress(self.TASK_PREPARE_FILES, self.TASK_PREPARE_FILES, title, body, next_enabled=True)
        self.card.set_content(
            title,
            body,
        ...
        self.set_current_focus("match_mode", title, body)

    def show_generate_step(self):
        if self._disposed:
            return
        self.begin_step("generate")
        self.configure_interaction("settings")
        self.ensure_settings_panel_open()
        title = I18n.get("tutorial_step_generate_title")
        body = I18n.get("tutorial_step_generate_body")
        self.set_tutorial_progress(self.TASK_PREPARE_FILES, self.TASK_PREPARE_FILES, title, body, next_enabled=True)
        self.card.set_content(
            title,
            body,
        ...
        self.set_current_focus("generate", title, body)

    def show_ocr_mode_step(self):
        if self._disposed:
            return
        self.begin_step("ocr_mode")
        self.configure_interaction("settings")
        self.ensure_settings_panel_open()
        title = I18n.get("tutorial_step_ocr_mode_title")
        body = I18n.get("tutorial_step_ocr_mode_body")
        self.set_tutorial_progress(self.TASK_OCR_MODE, self.TASK_OCR_MODE, title, body, next_enabled=True)
        self.card.set_content(title, body, back_enabled=True, next_enabled=True)
        self.set_current_focus("ocr_mode", title, body)

    def show_first_ocr_step(self):
        if self._disposed:
            return
        self.begin_step("first_ocr")
        self.configure_interaction("ocr")
        self.settings.hide_animated(force=True)
        self.ball.set_expanded(False)
        title = I18n.get("tutorial_step_first_ocr_title")
        body = I18n.get("tutorial_step_first_ocr_body")
        self.set_tutorial_progress(self.TASK_FIRST_OCR, self.TASK_FIRST_OCR, title, body, next_enabled=False)
        self.card.set_content(
            title,
            body,
            back_enabled=True,
            next_enabled=False,
            action_hint=I18n.get("tutorial_action_click_ocr"),
        )
        self.schedule_current(240, lambda: self.set_current_focus("ball", title, body))

    def handle_tutorial_ocr_triggered(self, *_args):
        if self._disposed or self.stage != "first_ocr":
            return
        self.completed_count = len(TutorialTaskPanel.TASK_KEYS)
        self.finish()

    def set_current_focus(self, target_name, title, body):
        self.current_target = target_name
        self.current_title = title
        self.current_body = body
        self._raised_focus_target = ""
        self._raised_card_target = ""
        if target_name in self.SETTINGS_TARGETS:
            self.settings.ensure_tutorial_target_visible(target_name)
            self.schedule_current(80, self.refresh_current_focus)
        self.refresh_current_focus()

    def refresh_current_focus(self, _anchor=None):
        if self._disposed:
            return
        if self.current_target in self.SETTINGS_TARGETS:
            target_rect = self.settings.tutorial_target_rect(self.current_target)
        else:
            target_rect = self.ball.tutorial_target_rect(self.current_target)
        if target_rect.isNull() or not target_rect.isValid():
            target_rect = self.ball.tutorial_target_rect("ball")
        focus_changed = (
            target_rect != self._last_focus_rect
            or self.current_target != self._last_focus_target
            or self.current_title != self._last_focus_title
            or self.current_body != self._last_focus_body
        )
        if focus_changed:
            self.overlay.set_focus(target_rect, self.current_title, self.current_body)
            self._last_focus_rect = QRect(target_rect)
            self._last_focus_target = self.current_target
            self._last_focus_title = self.current_title
            self._last_focus_body = self.current_body
        if self.current_target in self.SETTINGS_TARGETS and self._raised_focus_target != self.current_target:
            self.overlay.raise_()
            self.panel.raise_()
            self._raised_focus_target = self.current_target
        self.card.position_near(target_rect)
        if self._raised_card_target != self.current_target:
            self.card.raise_()
            self._raised_card_target = self.current_target

    def on_follow_tick(self):
        self.refresh_current_focus()
        self.check_menu_hover_progress()

    def check_menu_hover_progress(self):
        if self._disposed or self.stage != "menu":
            return
        if not (0 <= self.menu_index < len(self.MENU_ITEMS)):
            return
        target_name = self.MENU_ITEMS[self.menu_index][0]
        if self.ball.hovered != target_name:
            self._hover_confirm_target = None
            self._hover_confirm_started_at = 0.0
            return
        now = time.monotonic()
    ...
    def back_step(self):
        if self.stage == "menu":
            if self.menu_index > 0:
                self.begin_step("menu")
                self.menu_index -= 1
                self.show_current_menu_item()
            else:
                self.completed_count = self.TASK_EXPAND_MENU
                self.show_expand_step()
        elif self.stage == "open_settings":
            self.begin_step("menu")
            self.menu_index = len(self.MENU_ITEMS) - 1
            self.show_current_menu_item()
        elif self.stage == "prepare_files":
            self.completed_count = self.TASK_OPEN_SETTINGS
            self.show_open_settings_step()
        elif self.stage == "match_mode":
            self.completed_count = self.TASK_PREPARE_FILES
            self.show_prepare_files_step()
        elif self.stage == "generate":
            self.completed_count = self.TASK_PREPARE_FILES
            self.show_match_mode_step()
        elif self.stage == "ocr_mode":
            self.completed_count = self.TASK_PREPARE_FILES
            self.show_generate_step()
        elif self.stage == "first_ocr":
            self.completed_count = self.TASK_OCR_MODE
            self.show_ocr_mode_step()
    ...
    def next_step(self):
        if self.stage == "menu":
            ...
            self.completed_count = self.TASK_FIRST_OCR
            self.show_first_ocr_step()
    ...
    def dispose(self):
        ...
        try:
            self.ball.menuExpanded.disconnect(self.handle_menu_expanded)
            self.overlay.targetHovered.disconnect(self.handle_target_hovered)
            self.ball.ballMoved.disconnect(self.refresh_current_focus)
            self.ball.settingsRequested.disconnect(self.handle_settings_requested)
            self.settings.gameChanged.disconnect(self.handle_prepare_files_action)
            self.settings.languagesChanged.disconnect(self.handle_prepare_files_action)
            self.settings.smartFixRequested.disconnect(self.handle_prepare_files_action)
            self.ball.ocrChanged.disconnect(self.handle_tutorial_ocr_triggered)
        except (TypeError, RuntimeError):
            pass
        self.overlay.close()
        if not self._owns_overlay:
...
class SelectableOverlayText(QtWidgets.QTextEdit):
    ...
    def mouseReleaseEvent(self, event):
        if self._forwarding_drag:
            ...
            event.accept()
            return
        was_selecting = self._selecting_text
        if event.button() == Qt.LeftButton:
            self._selecting_text = False
        super().mouseReleaseEvent(event)
        if was_selecting and self is self.owner.debug_text_view:
            self.owner.flush_pending_debug_refresh()

    def wheelEvent(self, event):
        if self is self.owner.debug_text_view:
            super().wheelEvent(event)
            self.owner.sync_debug_scroll_from_view()
            self.owner.update()
            event.accept()
            return
        super().wheelEvent(event)
...
class SubtitleWindow(SoftWidget):
    def __init__(self, parent=None, floating=False):
        ...
        self.debug_scroll = 0
        self.debug_lines = []
        self._debug_text_cache = ""
        self._debug_refresh_pending = False
    ...
    def set_background_opacity(self, opacity):
        ...
        self._background_opacity = max(opacity_min, min(opacity_max, float(opacity)))
        self.update()
    ...
    def match_status_text(self):
        ...
        state_key = {
            ...
            "stale": "ui_dict_memory_stale",
            "missing_stale": "ui_dict_memory_stale",
    ...
    def set_controls_visible_immediate(self, visible):
        ...
        if self.controls_visible:
            ...
            self._subtitle_height = max(
                ...
                round(self.height() - self.controls_section_height),
            )
        else:
            self._controls_progress = 0.0
            self._subtitle_height = max(self.minimum_subtitle_height, self.height())
        self.setMinimumHeight(
            round(self.minimum_subtitle_height + self.controls_section_height * self._controls_progress)
        )
        self.resize(self.width(), round(self._subtitle_height + self.controls_section_height * self._controls_progress))
        self.refresh_text_views()
        self.update()
    ...
    def refresh_debug_text(self):
        ...
        if getattr(self.debug_text_view, "_selecting_text", False):
            self._debug_refresh_pending = self._debug_refresh_pending or text != self._debug_text_cache
            return
        if text != self._debug_text_cache:
            cursor = self.debug_text_view.textCursor()
            had_selection = cursor.hasSelection()
            anchor, position = cursor.anchor(), cursor.position()
            self.debug_text_view.setPlainText(text)
            if had_selection:
                max_position = len(text)
                cursor = self.debug_text_view.textCursor()
                cursor.setPosition(max(0, min(anchor, max_position)))
                cursor.setPosition(max(0, min(position, max_position)), QtGui.QTextCursor.KeepAnchor)
                self.debug_text_view.setTextCursor(cursor)
            self._debug_text_cache = text
        self.set_debug_view_scroll_from_line()
    ...
    def set_debug_view_scroll_from_line(self):
        visible_count = 5
        maximum = max(0, len(self.debug_lines) - visible_count)
        bar = self.debug_text_view.verticalScrollBar()
        if maximum <= 0 or bar.maximum() <= 0:
            bar.setValue(0)
        else:
            bar.setValue(round(bar.maximum() * self.debug_scroll / maximum))
        self.sync_debug_scroll_from_view()

    def sync_debug_scroll_from_view(self):
        visible_count = 5
        maximum = max(0, len(self.debug_lines) - visible_count)
        bar = self.debug_text_view.verticalScrollBar()
        if maximum <= 0 or bar.maximum() <= 0:
            ...
            self.debug_scroll = max(0, min(maximum, round(maximum * bar.value() / bar.maximum())))

    def flush_pending_debug_refresh(self):
        if not self._debug_refresh_pending:
            self.sync_debug_scroll_from_view()
            return
        self._debug_refresh_pending = False
        self.refresh_debug_text()
...
class SettingsWindow(SoftWidget):
    ...
    def __init__(self, initial_state=None):
        ...
        self._generate_text = ""
        self._dict_remote_updated_at = None
        self._dict_oldest_local_ctime = None
        self._dict_missing_mask = 0
        self._dict_stale_mask = 0
        self.tutorial_prevent_close = False
        self.tutorial_prevent_drag = False
        ...
        self.setStyleSheet("""
            ...
            QPushButton#generateButton[generateState="stale"],
            QPushButton#generateButton[generateState="missing_stale"] {
                background: #252936; border-color: #363b4c; color: #e9eaf3;
                font-weight: 600;
            }
            QPushButton#generateButton[generateState="missing"]:hover,
            QPushButton#generateButton[generateState="stale"]:hover,
            QPushButton#generateButton[generateState="missing_stale"]:hover {
        ...
        close.clicked.connect(lambda: self.hide_animated())
        ...
        self.reset_defaults_button = QtWidgets.QPushButton(I18n.get("ui_reset_defaults"))
        self.reset_defaults_button.setProperty("i18nKey", "ui_reset_defaults")
        sections.addWidget(self.reset_defaults_button)
    ...
    def default_initial_state():
        config = UserConfigManager.default_config()
        game = config["match"]["game"]
        source_lang = config["match"]["source_lang"]
        profile = Config.personalization_default(game, source_lang)
        return {
            "interface_language": config["ui"]["interface_language"],
            "ocr_mode": config["ocr"]["mode"],
            "auto_frequency_per_4s": config["ocr"]["auto_frequency_per_4s"],
            "detect_frequency": config["ocr"]["detect_frequency"],
            "detect_sensitivity": config["ocr"]["detect_sensitivity"],
            "game": game,
            "source_lang": source_lang,
            "target_lang": config["match"]["target_lang"],
            "match_mode": config["match"]["match_mode"],
            "similarity_threshold": config["match"]["similarity_threshold"],
            "player_nickname": profile["player_nickname"],
            "protagonist_gender": profile["protagonist_gender"],
            "auto_copy": config["output"]["auto_copy"],
            "auto_save": config["output"]["auto_save"],
            "subtitle_font_size": config["subtitle"]["font_size"],
        }

    def initial_value(self, key):
        return self.initial_state[key]

    def apply_state(self, state, *, preserve_interface_language=True):
        self.initial_state.update(state)
        controls = (
            self.run_mode,
            self.auto_frequency,
            self.detect_frequency,
            self.detect_sensitivity,
            self.game_combo,
            self.source_lang,
            self.target_lang,
            self.match_mode,
        ...
        if not preserve_interface_language:
            self.interface_language.setCurrentIndex(
                self.interface_language.findData(self.initial_value("interface_language"))
            )
        self.run_mode.setValue(self.initial_value("ocr_mode"))
        self.run_params.setCurrentIndex(self.initial_value("ocr_mode"))
        self.auto_frequency.setValue(self.initial_value("auto_frequency_per_4s"))
        self.detect_frequency.setValue(self.initial_value("detect_frequency"))
        self.detect_sensitivity.setValue(self.initial_value("detect_sensitivity"))
        self.game_combo.setCurrentIndex(self.game_combo.findData(self.initial_value("game")))
        self.source_lang.setCurrentIndex(self.source_lang.findData(self.initial_value("source_lang")))
        self.target_lang.setCurrentIndex(self.target_lang.findData(self.initial_value("target_lang")))
        self._source_lang_code = self.source_lang.currentData()
        self._target_lang_code = self.target_lang.currentData()
        self.match_mode.setValue(self.initial_value("match_mode"))
        self.threshold.setValue(self.initial_value("similarity_threshold"))
        self.player_nickname.setText(self.initial_value("player_nickname"))
        self.protagonist_gender.setCurrentIndex(
            self.protagonist_gender.findData(self.initial_value("protagonist_gender"))
        )
        self.auto_copy.setChecked(self.initial_value("auto_copy"))
        self.auto_save.setChecked(self.initial_value("auto_save"))
        self.subtitle_font_size.setValue(self.initial_value("subtitle_font_size"))
        del blockers
    ...
    def limit_combo_width(self, combo):
    ...
    def build_basic_section(self):
        ...
        self.interface_language.setCurrentIndex(
            self.interface_language.findData(
                self.initial_value("interface_language")
            )
        )
        ...
        font_size, self.subtitle_font_size = self.labeled_slider(
            ...
            font_min,
            font_max,
            self.initial_value("subtitle_font_size"),
    ...
    def build_ocr_section(self):
        ...
        ocr_mode = self.initial_value("ocr_mode")
        run_mode_widget, self.run_mode = self.choice_slider(
            ("ui_manual", "ui_auto", "ui_detect"), ocr_mode
        )
        ...
        auto_page, self.auto_frequency = self.labeled_slider(
            ...
            auto_min,
            auto_max,
            self.initial_value("auto_frequency_per_4s"),
        ...
        frequency, self.detect_frequency = self.labeled_slider(
            ...
            detect_min,
            detect_max,
            self.initial_value("detect_frequency"),
        ...
        sensitivity, self.detect_sensitivity = self.labeled_slider(
            ...
            sensitivity_min,
            sensitivity_max,
            self.initial_value("detect_sensitivity"),
        ...
        self.run_params.setCurrentIndex(ocr_mode)
    ...
    def build_game_language_row(self):
        ...
        self.limit_combo_width(self.game_combo)
        ...
        self.limit_combo_width(self.source_lang)
        self.limit_combo_width(self.target_lang)
        ...
        self.source_lang.setCurrentIndex(
            self.source_lang.findData(self.initial_value("source_lang"))
        )
        self.target_lang.setCurrentIndex(
            self.target_lang.findData(self.initial_value("target_lang"))
        )
        self._source_lang_code = self.source_lang.currentData()
        self._target_lang_code = self.target_lang.currentData()
    ...
    def build_match_section(self):
        ...
        threshold, self.threshold = self.labeled_slider(
            ...
            threshold_min,
            threshold_max,
            self.initial_value("similarity_threshold"),
            lambda value: I18n.get("unit_percent", value),
        )
        ...
        self.player_nickname = QtWidgets.QLineEdit(
            self.initial_value("player_nickname")
        )
        ...
        self.protagonist_gender.setCurrentIndex(
            self.protagonist_gender.findData(
                self.initial_value("protagonist_gender")
            )
        )
        ...
        self.dictionary_file_state = QtWidgets.QLabel(I18n.get("ui_dict_file_state_unknown"))
        self.dictionary_file_state.setObjectName("hint")
        self.dictionary_file_state.setWordWrap(True)
        layout.addWidget(self.dictionary_file_state)
    ...
    def build_output_section(self):
        ...
        row = QtWidgets.QHBoxLayout()
        row.setSpacing(10)
        self.auto_copy = self.output_toggle("ui_auto_copy", self.initial_value("auto_copy"))
        self.auto_save = self.output_toggle("ui_auto_save", self.initial_value("auto_save"))
    ...
    def start_group_drag(self, global_position):
        if self.tutorial_prevent_drag or self.follow_ball_ref is None or not self.isVisible():
            return
    ...
    def eventFilter(self, watched, event):
        ...
        if self.is_group_drag_surface(watched):
            if self.tutorial_prevent_drag and event.type() in (
    ...
    def _format_file_timestamp(self, timestamp):
        if not timestamp:
            return I18n.get("ui_dict_local_none")
        return datetime.fromtimestamp(timestamp).strftime("%Y-%m-%d %H:%M:%S")

    def set_dictionary_file_state(self, remote_updated_at, oldest_local_ctime, missing_mask, stale_mask):
        self._dict_remote_updated_at = remote_updated_at
        self._dict_oldest_local_ctime = oldest_local_ctime
        self._dict_missing_mask = int(missing_mask or 0)
        self._dict_stale_mask = int(stale_mask or 0)
        local_text = self._format_file_timestamp(self._dict_oldest_local_ctime)
        if self._dict_remote_updated_at:
            remote_text = self._format_file_timestamp(self._dict_remote_updated_at)
            text = I18n.get("ui_dict_file_state", remote_text, local_text)
        else:
            text = I18n.get("ui_dict_file_remote_unknown", local_text)
        self.dictionary_file_state.setText(text)
    ...
    def retranslate_ui(self):
        ...
        self.update_output_toggle(self.auto_copy, self.auto_copy.isChecked())
        self.update_output_toggle(self.auto_save, self.auto_save.isChecked())
        self.set_dictionary_file_state(
            self._dict_remote_updated_at,
            self._dict_oldest_local_ctime,
            self._dict_missing_mask,
            self._dict_stale_mask,
        )
    ...
    def set_generate_state(self, state, text=""):
        if state not in {"off", "pending", "loading", "missing", "stale", "missing_stale", "ready", "error"}:
            state = "loading"
    ...
    def _refresh_match_interaction(self):
        ...
        self.generate_button.setEnabled(state in {"missing", "stale", "missing_stale"} and not self._dict_locked)
        fallback_text = {
            ...
            "stale": I18n.get("ui_match_data_stale"),
            "missing_stale": I18n.get("ui_match_data_missing_stale"),
            "ready": I18n.get("ui_match_data_ready"),
    ...
    def tutorial_target_rect(self, name):
        ...
        if name == "panel":
            return widget_rect(self)
        if name == "game_language":
            rects = [
                widget_rect(getattr(self, "game_combo", None)),
                widget_rect(getattr(self, "source_lang", None)),
                widget_rect(getattr(self, "target_lang", None)),
            ]
            valid_rects = [rect for rect in rects if rect.isValid() and not rect.isNull()]
            if not valid_rects:
                return QRect()
            result = QRect(valid_rects[0])
            for rect in valid_rects[1:]:
                result = result.united(rect)
            return result.adjusted(-8, -8, 8, 8)
        if name == "generate":
            return widget_rect(getattr(self, "generate_button", None))
        if name == "match_mode":
            return widget_rect(getattr(self, "match_mode", None))
        if name == "ocr_mode":
            return widget_rect(getattr(self, "run_mode", None))
        return QRect()

    def tutorial_focus_widget(self, name):
        if name == "game_language":
            return getattr(self, "game_combo", None)
        if name == "generate":
            return getattr(self, "generate_button", None)
        if name == "match_mode":
            return getattr(self, "match_mode", None)
        if name == "ocr_mode":
            return getattr(self, "run_mode", None)
        return None

    def ensure_tutorial_target_visible(self, name):
        if name == "panel":
            return
        widget = self.tutorial_focus_widget(name)
        if widget is None or not hasattr(self, "scroll_area"):
            return
        self.scroll_area.ensureWidgetVisible(widget, 24, 36)

    def show_near(self, ball):
        if self.opening:
            return
        if self.isVisible():
            if self.tutorial_prevent_close:
                self.raise_()
                return
            self.hide_animated()
            return
    ...
    def hide_animated(self, force=False):
        if self.tutorial_prevent_close and not force:
            self.raise_()
            return
    ...
    def mousePressEvent(self, event):
        if self.tutorial_prevent_drag and event.button() == Qt.LeftButton:
            event.accept()
            return
        if event.button() == Qt.LeftButton:
            self.start_group_drag(event.globalPosition())
    ...
    def mouseMoveEvent(self, event):
        if self.tutorial_prevent_drag and event.buttons() & Qt.LeftButton:
            event.accept()
            return
...
class FaintOCRApplication(QtCore.QObject):
    ...
    FLOATING_ERROR_HOLD_MS = 5000

    def __init__(self):
        super().__init__()
        self.user_config = UserConfigManager.load()
        I18n.current_lang = self.user_config["ui"]["interface_language"]
        self.controller = OCRController()
        self.apply_user_config_to_controller()
        ocr_config = self.user_config["ocr"]
        output_config = self.user_config["output"]
        subtitle_config = self.user_config["subtitle"]
        match_config = self.user_config["match"]
        self.ball = FloatingBall(floating=True)
        self.subtitle = SubtitleWindow(floating=True)
        self.settings = SettingsWindow({
            "interface_language": self.user_config["ui"]["interface_language"],
            "ocr_mode": ocr_config["mode"],
            "auto_frequency_per_4s": ocr_config["auto_frequency_per_4s"],
            "detect_frequency": ocr_config["detect_frequency"],
            "detect_sensitivity": ocr_config["detect_sensitivity"],
            "game": match_config["game"],
            "source_lang": match_config["source_lang"],
            "target_lang": match_config["target_lang"],
            "match_mode": match_config["match_mode"],
            "similarity_threshold": match_config["similarity_threshold"],
            "player_nickname": self.controller.player_nickname,
            "protagonist_gender": self.controller.protagonist_gender,
            "auto_copy": output_config["auto_copy"],
            "auto_save": output_config["auto_save"],
            "subtitle_font_size": subtitle_config["font_size"],
        })
        self.ball.settings_window = self.settings

        self.ocr_region = tuple(ocr_config["region"])
        self.ocr_mode = ocr_config["mode"]
        self.auto_ocr_enabled = Config.DEFAULTS["auto_ocr_enabled"]
        self.detect_ocr_enabled = Config.DEFAULTS["detect_ocr_enabled"]
        self.auto_ocr_interval = Config.auto_frequency_to_interval(ocr_config["auto_frequency_per_4s"])
        self.detect_frequency = ocr_config["detect_frequency"]
        self.detect_sensitivity = ocr_config["detect_sensitivity"]
        self.auto_copy = output_config["auto_copy"]
        self.auto_save = output_config["auto_save"]
        self.subtitle_visible = subtitle_config["visible"]
        self.subtitle.set_subtitle_font_size(subtitle_config["font_size"])
        self.subtitle.set_background_opacity(subtitle_config["opacity"])
        self._last_matched_output_text = None
        self._manual_ocr_pending = 0
        self._cleaned_up = False
        self._floating_error_protected_until = 0.0
        self.selection_overlay = None
        self.region_overlay = None
        self.welcome_window = None
        self.tutorial_overlay = None
        self.tutorial_guide = None
        ...
        self.log_bridge = FloatingDebugLogBridge(self)
        self.log_bridge.install(self.append_debug)
        self.initialize_positions()
        self.controller._refresh_ui_state()

    def apply_user_config_to_controller(self):
        match_config = self.user_config["match"]
        self.controller.current_game = match_config["game"]
        self.controller.source_lang = match_config["source_lang"]
        self.controller.target_lang = match_config["target_lang"]
        self.controller.match_mode = match_config["match_mode"]
        self.controller.similarity_threshold = match_config["similarity_threshold"]
        for key, profile in self.user_config["personalization_profiles"].items():
            if "|" not in key:
                continue
            game_key, source_lang = key.split("|", 1)
            self.controller.personalization_profiles[(game_key, source_lang)] = dict(profile)
        self.controller._apply_personalization_profile()
        self.controller.text_matcher.clear_cache()
    ...
    def restore_default_settings(self):
        defaults = UserConfigManager.default_config()
        default_state = SettingsWindow.default_initial_state()
        default_state["interface_language"] = I18n.current_lang

        self.stop_active_mode()
        self.ocr_region = tuple(defaults["ocr"]["region"])
        self.ocr_mode = defaults["ocr"]["mode"]
        self.auto_ocr_interval = Config.auto_frequency_to_interval(defaults["ocr"]["auto_frequency_per_4s"])
        self.detect_frequency = defaults["ocr"]["detect_frequency"]
        self.detect_sensitivity = defaults["ocr"]["detect_sensitivity"]
        self.auto_copy = defaults["output"]["auto_copy"]
        self.auto_save = defaults["output"]["auto_save"]
        self.subtitle_visible = defaults["subtitle"]["visible"]

        self.controller.current_game = defaults["match"]["game"]
        self.controller.source_lang = defaults["match"]["source_lang"]
        self.controller.target_lang = defaults["match"]["target_lang"]
        self.controller.match_mode = defaults["match"]["match_mode"]
        self.controller.similarity_threshold = defaults["match"]["similarity_threshold"]
        self.controller.personalization_profiles = {
            (game_key, source_lang): dict(profile)
            for game_key, languages in Config.PERSONALIZATION_DEFAULTS.items()
            for source_lang, profile in languages.items()
        }
        self.controller._apply_personalization_profile()
        self.controller.text_matcher.clear_cache()
        self.controller._reset_dict_state()

        self.subtitle.set_subtitle_font_size(defaults["subtitle"]["font_size"])
        self.subtitle.set_background_opacity(defaults["subtitle"]["opacity"])
        self.subtitle.setVisible(self.subtitle_visible)
        self.settings.apply_state(default_state, preserve_interface_language=True)
        self.update_subtitle_context()
        self.subtitle.update_match_mode(self.controller.match_mode)
        self.ball.ocr_momentary = self.ocr_mode == 0
        self.sync_ball_state(False)
        self._last_matched_output_text = None
        self.show_settings_status(I18n.get("ui_reset_defaults"))
    ...
    def refresh_settings_display(self):
        ...
        for slider in (
            ...
            if hasattr(slider, "_value_formatter"):
                slider._value_label.setText(slider._value_formatter(slider.value()))
        settings.update_output_toggle(settings.auto_copy, settings.auto_copy.isChecked())
        settings.update_output_toggle(settings.auto_save, settings.auto_save.isChecked())
    ...
    def connect_signals(self):
        ...
        self.settings.resetDefaultsRequested.connect(self.restore_default_settings)
        ...
        self.controller.dict_file_state_signal.connect(self.settings.set_dictionary_file_state)
    ...
    def initialize_positions(self):
        screen = QApplication.primaryScreen().availableGeometry()
        window_config = self.user_config["windows"]
        ball_anchor = self._point_from_config(window_config.get("floating_ball_anchor"))
        if ball_anchor is None:
            ball_anchor = QPoint(screen.right() - 75, screen.center().y() - 170)
        ball_anchor = self._fit_ball_anchor(ball_anchor)
        self.ball.menu_direction = self.ball.preferred_menu_direction(ball_anchor)
        self.ball.set_ball_anchor_global(ball_anchor)

        subtitle_geometry = self._rect_from_config(window_config.get("subtitle_geometry"))
        if subtitle_geometry is None:
            self.subtitle.move(
                screen.center().x() - self.subtitle.width() // 2,
                screen.bottom() - self.subtitle.height() - 70,
            )
        else:
            self.subtitle.setGeometry(self._fit_subtitle_geometry(subtitle_geometry))
        self.subtitle.set_controls_visible_immediate(window_config.get("subtitle_controls_visible", True))

    def _point_from_config(self, value):
        if not isinstance(value, (list, tuple)) or len(value) != 2:
            return None
        return QPoint(int(value[0]), int(value[1]))

    def _rect_from_config(self, value):
        if not isinstance(value, (list, tuple)) or len(value) != 4:
            return None
        return QRect(int(value[0]), int(value[1]), int(value[2]), int(value[3]))

    def _fit_ball_anchor(self, anchor):
        area = self.ball.drag_bounds_global()
        ball_rect = self.ball.ball_rect()
        max_x = area.right() - int(ball_rect.width()) + 1
        max_y = area.bottom() - int(ball_rect.height()) + 1
        return QPoint(
            max(area.left(), min(anchor.x(), max_x)),
            max(area.top(), min(anchor.y(), max_y)),
        )

    def _fit_subtitle_geometry(self, geometry):
        area = QApplication.primaryScreen().availableGeometry()
        width = max(self.subtitle.minimumWidth(), min(geometry.width(), self.subtitle.maximumWidth()))
        height = max(self.subtitle.minimum_subtitle_height, min(geometry.height(), self.subtitle.maximumHeight()))
        max_x = area.right() - width + 1
        max_y = area.bottom() - height + 1
        return QRect(
            max(area.left(), min(geometry.x(), max_x)),
            max(area.top(), min(geometry.y(), max_y)),
            width,
            height,
        )
    ...
    def show(self):
        ...
        if not icon.isNull():
            ...
            logger.warning(I18n.get("log_icon_load_failed"))
        self.subtitle.setVisible(self.subtitle_visible)
        for widget, label in (
            ...
            apply_capture_exclusion(widget, label)
        QTimer.singleShot(500, self.maybe_show_tutorial)

    def tutorial_state(self):
        tutorial = self.user_config.get("tutorial")
        if not isinstance(tutorial, dict):
            tutorial = UserConfigManager.default_config()["tutorial"]
            self.user_config["tutorial"] = tutorial
        return tutorial

    def save_tutorial_state(self, *, completed=None, welcome_completed=None, last_step=None):
        tutorial = self.tutorial_state()
        if completed is not None:
            tutorial["completed"] = bool(completed)
        if welcome_completed is not None:
            tutorial["welcome_completed"] = bool(welcome_completed)
        if last_step is not None:
            tutorial["last_step"] = int(last_step)
        UserConfigManager.save(self.create_user_config_snapshot())

    def maybe_show_tutorial(self):
        tutorial = self.tutorial_state()
        if tutorial.get("completed", False):
            return
        if not tutorial.get("welcome_completed", False):
            self.show_tutorial_welcome()
            return
        self.start_tutorial()

    def show_tutorial_welcome(self):
        if self.welcome_window is not None and self.welcome_window.isVisible():
            self.welcome_window.raise_()
            return
        self.show_tutorial_overlay()
        self.welcome_window = TutorialWelcomeWindow(I18n.current_lang)
        self.welcome_window.languageChanged.connect(self.handle_tutorial_language_changed)
        self.welcome_window.startRequested.connect(self.handle_tutorial_welcome_start)
        self.welcome_window.skipRequested.connect(self.handle_tutorial_welcome_skip)
        self.welcome_window.show()
        self.welcome_window.raise_()

    def show_tutorial_overlay(self):
        if self.tutorial_overlay is None:
            self.tutorial_overlay = TutorialFocusOverlay()
        self.tutorial_overlay.set_focus(QRect(), "", "")
        self.tutorial_overlay.show()
        self.tutorial_overlay.raise_()

    def close_tutorial_overlay(self):
        if self.tutorial_overlay is None:
            return
    ...
    def close_tutorial_welcome(self):
    ...
    def apply_tutorial_language(self, language):
        if language not in I18n.TEXT:
            return
        self.set_interface_language(language, force=True)
        self.user_config.setdefault("ui", {})["interface_language"] = language

    def handle_tutorial_language_changed(self, language):
        self.apply_tutorial_language(language)

    def handle_tutorial_welcome_start(self, language):
        self.close_tutorial_welcome()
        self.apply_tutorial_language(language)
        self.save_tutorial_state(welcome_completed=True, last_step=1)
        self.start_tutorial()

    def handle_tutorial_welcome_skip(self, language):
        self.close_tutorial_welcome()
        self.apply_tutorial_language(language)
        self.close_tutorial_overlay()
        self.save_tutorial_state(completed=True, welcome_completed=True, last_step=8)

    def start_tutorial(self):
        if self.tutorial_guide is not None:
            self.tutorial_guide.dispose()
            self.tutorial_guide.deleteLater()
        self.ball.raise_()
        if self.tutorial_overlay is None:
            self.tutorial_overlay = TutorialFocusOverlay()
        self.tutorial_guide = TutorialGuide(self, self.tutorial_overlay)
        self.tutorial_guide.finished.connect(self.handle_tutorial_finished)
        self.tutorial_guide.skipped.connect(self.handle_tutorial_skipped)
        self.tutorial_guide.start()

    def handle_tutorial_finished(self):
        guide = self.tutorial_guide
        self.tutorial_guide = None
        if guide is not None:
            guide.deleteLater()
        self.close_tutorial_overlay()
        self.save_tutorial_state(completed=True, welcome_completed=True, last_step=8)
        self.show_floating_status(I18n.get("tutorial_completed"))

    def handle_tutorial_skipped(self):
        guide = self.tutorial_guide
        self.tutorial_guide = None
        if guide is not None:
            guide.deleteLater()
    ...
    def toggle_subtitle(self):
        self.subtitle_visible = not self.subtitle.isVisible()
        self.subtitle.setVisible(self.subtitle_visible)
    ...
    def set_auto_ocr_enabled(self, enabled):
        ...
        if enabled:
            ...
            self.ocr_timer.stop()
            self.show_floating_status(I18n.get("status_auto_ocr_off"))
    ...
    def set_detect_ocr_enabled(self, enabled):
        ...
        if enabled:
            ...
            self.controller.stop_detection()
        self.show_floating_status(I18n.get("status_detect_ocr_on" if enabled else "status_detect_ocr_off"))
        self.sync_ball_state(enabled)
    ...
    def set_interface_language(self, language, force=False):
        if language not in I18n.TEXT or (language == I18n.current_lang and not force):
            return
        changed = language != I18n.current_lang
        ...
        language_index = self.settings.interface_language.findData(language)
        if language_index >= 0 and self.settings.interface_language.currentData() != language:
            language_blocker = QtCore.QSignalBlocker(self.settings.interface_language)
            self.settings.interface_language.setCurrentIndex(language_index)
    ...
    def handle_region_selected(self, rect):
        ...
        self.show_floating_status(message)
    ...
    def handle_ocr_result(self, is_success, main_text, status_info, allow_auto_output, task_source):
        if not is_success:
            self.show_floating_status(main_text, error=True)
        elif not main_text:
            self.subtitle.update_text("...")
            self.show_floating_status(self._build_ocr_summary(task_source, status_info))
        else:
    ...
    def process_output(self, text, allow_auto_output):
        ...
        if self.auto_save:
            try:
                ...
                message = I18n.get("ui_save_failed", exc)
                logger.error(message)
                self.show_floating_status(message, error=True)
        if is_matched:
    ...
    def show_floating_status(self, message, *, error=False):
        if not message:
            return False
        now = time.monotonic()
        if not error and now < self._floating_error_protected_until:
            return False
        ...
        if error:
            self._floating_error_protected_until = now + self.FLOATING_ERROR_HOLD_MS / 1000.0
        elif now >= self._floating_error_protected_until:
            self._floating_error_protected_until = 0.0
        return True
    ...
    def handle_detect_status(self, message):
        ...
        if (
                ...
                or message == I18n.get("status_detect_review_failed")
    ...
    def handle_dict_state(self, state, data_list, current_level, target_level):
        ...
        messages = {
            ...
            "STALE": I18n.get("state_stale"),
            "MISSING_STALE": I18n.get("state_missing_stale"),
        ...
        button_text = "" if generate_state in {"missing", "stale", "missing_stale"} else message
        self.settings.set_generate_state(generate_state, button_text)
        self.subtitle.update_match_data_state(generate_state)
        self.show_settings_status(message)
        self.show_floating_status(message, error=(state == "ERROR"))

    def create_user_config_snapshot(self):
        subtitle_geometry = self.subtitle.geometry()
        ball_anchor = self.ball.ball_anchor_global()
        if (
            self.settings.original_ball_anchor is not None
            and not self.settings.user_moved_ball
        ):
            ball_anchor = QPoint(self.settings.original_ball_anchor)
        profiles = {
            f"{game_key}|{source_lang}": dict(profile)
            for (game_key, source_lang), profile in self.controller.personalization_profiles.items()
        }
        return {
            "config_version": UserConfigManager.CONFIG_VERSION,
            "ui": {
                "interface_language": I18n.current_lang,
            },
            "ocr": {
                "mode": self.ocr_mode,
                "auto_frequency_per_4s": Config.interval_to_auto_frequency(self.auto_ocr_interval),
                "detect_frequency": self.detect_frequency,
                "detect_sensitivity": self.detect_sensitivity,
                "region": list(self.ocr_region),
            },
            "match": {
                "game": self.controller.current_game,
                "source_lang": self.controller.source_lang,
                "target_lang": self.controller.target_lang,
                "match_mode": self.controller.match_mode,
                "similarity_threshold": self.controller.similarity_threshold,
            },
            "output": {
            ...
            "subtitle": {
                "font_size": self.subtitle.subtitle_font_size,
                "opacity": self.subtitle._background_opacity,
                "visible": self.subtitle_visible,
            },
            "windows": {
                "floating_ball_anchor": [ball_anchor.x(), ball_anchor.y()],
                "subtitle_geometry": [
                    subtitle_geometry.x(),
                    subtitle_geometry.y(),
                    subtitle_geometry.width(),
                    subtitle_geometry.height(),
                ],
                "subtitle_controls_visible": self.subtitle.controls_visible,
            },
            "tutorial": dict(self.tutorial_state()),
            "personalization_profiles": profiles,
        }
