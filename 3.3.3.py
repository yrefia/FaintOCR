...
class I18n:
    ...
    TEXT = {
        "zh_CN": {
            ...
            "ui_status_connected": "FaintOCR 3.3.3 已连接 OCR 控制器",
            ...
            "log_prefix_ok": "前缀索引生成成功: Tk{0}",
            "log_prefix_failed": "批量前缀生成发生严重失败: {0}",
            ...
            "log_cleanup": "FaintOCR 3.3.3 资源清理完成",
        ...
        "en_US": {
            ...
            "ui_status_connected": "FaintOCR 3.3.3 connected to OCR controller",
            ...
            "log_prefix_ok": "Prefix index generated: Tk{0}",
            "log_prefix_failed": "Batch prefix generation failed: {0}",
            ...
            "log_cleanup": "FaintOCR 3.3.3 resources cleaned up",
            ...
            "interface_zh_CN": "Simplified Chinese",
        ...
        try:
            ...
            for N, file_path in missing_tokens:
                ...
                if cls._save_file_atomic(file_path, prefixes, indent=False):
                    logger.debug(I18n.get("log_prefix_ok", N))
                    results[N] = prefixes
                else:
            ...
            logger.error(I18n.get("log_prefix_failed", e))
            return None
...
class OCRWorker(QtCore.QRunnable):
    ...
    def run(self):
        try:
            ...
            if raw_text is None:
                logger.trace(I18n.get("log_ocr_fallback"))
...
class TextMatcher:
    ...
    def __init__(self, cache_size=3, len_cache_size=10):  # <--- [修改] 增加 len_cache_size，默认给10条足够了
        ...
        self.log_decay = math.log(self.decay_factor)  # 预先计算 ln(0.9)
    ...
    def _match_prefix_by_length(self, ocr_text, similarity_threshold):
        ...
        if target_len not in self.len_cache:
            ...
            # ✅ 核心修改：使用 target_len + 1 进行截取，确保前缀匹配时相似度恒不为 100
            self.len_cache[target_len] = {
                'data': [k[:target_len + 1] for k in self.keys_list],
                'score': 0.0,
                'last_step': self.global_step  # 记录初始创建时的绝对时钟
            }
    ...
    def _match_prefix_text(self, ocr_text, similarity_threshold):
        """前缀匹配模式 (重构后)"""
        if not self.text_map or not ocr_text.strip():
            return None, None, 0
...
class OCRTaskManager(QtCore.QObject):
    ...
    def submit_task(self, task_data):
        ...
        logger.trace(I18n.get("log_task_submit", len(self.task_queue), self.active_tasks))
        self._process_queue()
...
class OCRController(QtCore.QObject):
    ...
    def stop_detection(self):
        ...
        if had_worker:
            for signal in (self.detect_start_signal, self.detect_resume_signal, self.detect_update_signal):
                try:
                    signal.disconnect()
    ...
    def _on_ocr_task_completed(self, is_success, main_text, status_info, allow_auto_output, task_source,
                               detect_session_id, perf_metrics):
        if task_source == "detect" and detect_session_id != self.detect_session_id:
...
class SubtitleWindow(SoftWidget):
    def __init__(self, parent=None, floating=False):
        ...
        self.match_text = I18n.get(
            ("ui_match_off", "ui_match_exact", "ui_match_prefix", "ui_match_regex")[
    ...
    def update_match_mode(self, mode):
        self.match_text = I18n.get(("ui_match_off", "ui_match_exact", "ui_match_prefix", "ui_match_regex")[mode])
        self.update()
    ...
    def paintEvent(self, _event):
        ...
        subtitle_center_y = self._subtitle_height / 2
        ...
        if self._controls_progress > 0.01:
            ...
            center_controls_x = max(310, self.width() / 2 - 217)
            for rect, text in [
                (QRectF(center_controls_x, controls_y - 13, 110, 26), self.match_text),
                (QRectF(center_controls_x + 122, controls_y - 13, 110, 26), self.game_text),
                (QRectF(center_controls_x + 244, controls_y - 13, 190, 26), self.language_text),
            ]:
            ...
            p.setPen(color("#aeb3c5"))
            for index, text in enumerate(self.debug_lines[self.debug_scroll:self.debug_scroll + visible_count]):
                p.drawText(
                    QRectF(debug.left() + 12, debug.top() + 25 + index * 15, debug.width() - 32, 15),
                    Qt.AlignVCenter,
                    text,
                )
            if max_scroll:
...
class SettingsWindow(SoftWidget):
    ...
    def __init__(self):
        ...
        self.setStyleSheet("""
            ...
            QComboBox, QSlider { color: #d2d4df; }
            QComboBox { background: #252936; border: 1px solid #363b4c; border-radius: 8px;
            ...
            QPushButton#generateButton[dictReady="true"] { background: #1c2824; border-color: #3f6b59;
                                                           color: #83b99e; font-weight: 600; }
    ...
    def build_basic_section(self):
        ...
        self.interface_language.addItem(I18n.get("interface_zh_CN"), "zh_CN")
        self.interface_language.addItem(I18n.get("interface_en_US"), "en_US")
    ...
    def emit_languages(self):
        self.languagesChanged.emit(
            self.source_lang.currentData() or self.source_lang.currentText(),
    ...
    def retranslate_ui(self):
        ...
        for index in range(self.interface_language.count()):
            ...
            self.interface_language.setItemText(index, I18n.get(f"interface_{language}"))
    ...
    def _refresh_match_interaction(self):
        ...
        self.generate_button.setProperty("dictReady", self._match_ready)
        self.generate_button.setEnabled(not self._dict_locked and not self._match_ready)
        button_text = I18n.get("ui_match_data_ready" if self._match_ready else "ui_generate_match_data")
        self.generate_button.setText(button_text)
        self.generate_button.setToolTip(button_text if self._match_ready else "")
...
class FaintOCRApplication(QtCore.QObject):
    ...
    def set_ocr_enabled(self, enabled):
        if self.ocr_mode == 0:
            if enabled:
                ...
                self.controller.request_ocr(self.ocr_region, task_source="manual")
            else:
    ...
    def set_languages(self, source, target):
        if source == target:
            target_index = (self.settings.target_lang.currentIndex() + 1) % self.settings.target_lang.count()
            blocker = QtCore.QSignalBlocker(self.settings.target_lang)
            self.settings.target_lang.setCurrentIndex(target_index)
            del blocker
            target = self.settings.target_lang.currentData()
        self._last_matched_output_text = None
    ...
    def set_match_mode(self, mode):
        ...
        match_name = I18n.get(("ui_match_off", "ui_match_exact", "ui_match_prefix", "ui_match_regex")[mode])
    ...
    def handle_dict_state(self, state, data_list, current_level, target_level):
        messages = {
            ...
            "ERROR": data_list[0] if data_list else I18n.get("state_error"),
        }
        message = messages.get(state, state)
        self.settings.set_match_ready(state == "READY")
        self.show_settings_status(message)
