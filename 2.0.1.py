...
# - 修复思路2：先内存后硬盘
# - 何时可以if反向？没有任性条件时 a=1 b=2 == b=2 a=1 除非 a=1会时b不重要
...
class TextMapLoader:
    ...
    def handle_task(cls, game_key, source_lang, target_lang, mode, callbacks=None):
        ...
        if is_repair and task_mask > 0:
            task_mask = (1 << task_mask.bit_length()) - 1

        # [修改] 删除了毫无必要的 target_raw
        target_map = bool(task_mask & (1 << 1))  # 2^1 = 2
        target_prefix = bool(task_mask & (1 << 2))  # 2^2 = 4
    ...
    def _merge_and_save_map(cls, game_key, source_lang, target_lang, progress_cb, percent_cb, cancel_cb):
        ...
        try:
            ...
            for i, (k, s) in enumerate(src_d.items()):
                ...
                if k in tgt_d:
                    # [修改] 调用清洗函数处理 Source (OCR 识别的源语言)，传入 game_key 和 source_lang
                    s_tx = cls.clean_unity_rich_text(s, game_key, source_lang)
                    # [修改] 调用清洗函数处理 Target (显示的翻译文本)，传入 game_key 和 target_lang
...
class OCRController(QtCore.QObject):
    ...
    def __init__(self):
        ...
        # [修改] 0表示无数据。掩码规则: 1=Raw, 2=Map, 4=Prefix
        self.missing_raw = []
        self.missing_map = []
        self.missing_prefix = []
    ...
    def run_smart_fix(self):
        ...
        # 告诉 UI 准备进入加载/生成状态
        required_mask = self._get_required_mask(self.match_mode)
        self.ui_state_signal.emit(DictState.LOADING, [], self.current_data_mask, required_mask)

        # 位运算计算缺失内容
        missing_mask = required_mask & ~self.current_data_mask

        # [修改] 统一口径后，缺失掩码 missing_mask 直接就是 Worker 需要的 handle_mode！
        # 负数代表修复模式
        # [新增] 强制进行下位包含操作 (Downward Inclusion)
        # 因为是生成/修复任务，高级别数据的生成必须依赖低级别数据的加载校验
        if missing_mask > 0:
            # [修改] 数学之美：基于最高有效位，瞬间生成满载下位掩码
            # 如果未来新增 AI字典(1<<3=8)，8.bit_length()=4，(1<<4)-1 = 15(8+4+2+1)，无需修改任何代码！
            missing_mask = (1 << missing_mask.bit_length()) - 1
            handle_mode = -missing_mask
        else:
            handle_mode = 0
    ...
    def _get_required_mask(self, mode):
        """将业务需求(Mode)映射为数据需求掩码 (2^1:Map, 2^2:Prefix)"""
        if mode in (1, 2):
            return (1 << 1)  # 2^1
        elif mode == 3:
            return (1 << 1) | (1 << 2)  # 2^1 + 2^2 = 6
        return 0
    ...
    def _on_worker_finished(self, handle_game, handle_source_lang, handle_target_lang, handle_mode):
        ...
        # 1. 记录当次操作返回的文件状态
        #   - 检查模式(0)下: 代表纯粹的硬盘物理状态
        #   - 加载/修复模式(!=0)下: 代表本次读取/生成是否成功
        ...
        # 2. 严格的内存状态管理
        if handle_mode != 0:
            # 注入数据：TextMatcher.set_data 内部有防御机制，传入 None 不会覆盖旧内存
            self.text_matcher.set_data(text_map, keys_list, prefix_dict)

            task_mask = abs(handle_mode)
            # --- Map 内存状态 ---
            # 只有明确要求加载 (task_mask & 2) 并且 成功读取 (not missing_map)，才追加内存掩码。
            # 如果读取失败，旧数据仍在内存中，掩码保持原样，不作剥离。
            if (task_mask & (1 << 1)) and not self.missing_map:
                self.current_data_mask |= (1 << 1)
            # --- Prefix 内存状态 ---
            if (task_mask & (1 << 2)) and not self.missing_prefix:
                self.current_data_mask |= (1 << 2)
    ...
    def _refresh_ui_state(self):
        logger.info(f"当前内存状态: {self.current_data_mask}")
        ...
        # ==========================================
        # 方向一共识落地：内存至上 (Memory First)
        # ==========================================
        if missing_mask == 0:
            # 只要内存满足需求，无视任何硬盘缺失，直接宣判就绪
            self.ui_state_signal.emit(DictState.READY, [], self.current_data_mask, required_mask)
            return

        # --- 以下是暂存的逻辑（保留 missing_raw，留待后续讨论） ---
        current_missing = []
        if self.missing_raw:
            current_missing.extend(self.missing_raw)

        if (missing_mask & (1 << 1)) and self.missing_map:
            current_missing.extend(self.missing_map)

        if (missing_mask & (1 << 2)) and self.missing_prefix:
            current_missing.extend(self.missing_prefix)

        if len(current_missing) > 0:
            self.ui_state_signal.emit(DictState.MISSING, current_missing, 0, 0)
        else:
            self.ui_state_signal.emit(DictState.PENDING, [], self.current_data_mask, required_mask)
