...
# 3.6.0
# 新增 UserConfig.json 本地用户配置持久化：启动时读取一次，关闭时写入一次。
# 持久化基础设置、OCR 参数、匹配参数、输出偏好、个性化资料，以及悬浮球/悬浮窗位置与悬浮窗尺寸。

# 3.6.1
# 修复从配置恢复到右展开位置时，首次悬停悬浮球会短暂跳动的问题。
# 优化设置界面英文布局，避免英文控件最小宽度撑出设置面板。

# 3.6.2
# 撤回 3.6.1 对设置面板控件尺寸策略的激进调整，恢复原有紧凑布局。
# 启动时先以中文构建设置界面，再按用户配置切换界面语言，规避英文直启导致的宽度溢出。

# 3.6.2b
# 保留 3.6.1 的悬浮球首次悬停跳动修复。
# 设置面板 UI 与启动语言流程恢复为 3.6.0 行为。
# 使用“combo.setMaximumWidth(120)”限制语言下拉框宽度，避免英文直启时 Simplified Chinese 撑大设置面板。

# 3.6.3
# 修复读取用户配置后，设置面板中滑条数值和输出开关文本未同步刷新的问题。
# 修复关闭时保存悬浮窗显示状态总是 False 的问题。

# 3.6.4
# 设置面板顶部新增“恢复默认设置”，可重置除界面语言外的配置。
# 修复设置面板主动避让悬浮球且用户未拖动组合体时，关闭应用保存避让坐标的问题。
# 修改“self.auto_load_timer.start()”的值为“1200”

# 3.6.5
# 玩家输入的昵称改为按游戏唯一保存，同一游戏下所有源语言共用用户昵称。
# 默认昵称仍保留“游戏 + 语言”的本地化默认配置。

# 3.6.6
# 首次切换到新游戏时读取 GitLab 仓库最后更新时间并缓存于内存。
# 检查本地字典文件时同步比较创建时间，过旧时提示可选修复，同时允许继续使用本地旧版本。

# 3.6.7
# 字典任务相关事件统一输出到悬浮窗调试框，包括任务调度、状态机流转和远程仓库更新时间。

# 3.6.8
# 重构远程仓库更新时间与本地文件创建时间检查：字典任务返回 Missing/Stale 双掩码。
# 设置面板 D 区新增远程/本地文件状态显示，缺失与可更新共用生成匹配数据按钮入口。

# 3.6.8b
# 修复修复 Map 时未先刷新过期 Raw 依赖的问题：各层级在修复前按实际展开任务逐项检查并删除过期文件。

# 3.6.8c
# 过期判断改为使用文件创建时间与修改时间中的更早值，并移除 Stale 掩码返回中的原始任务掩码限制。

# 3.7.0
# Fix floating debug/control bar collapsed-state restore, move GitLab checks into dictionary tasks, and mirror logger status to the floating debug box.

# 3.7.1
# Remove the unused remote update helper, route all logger levels to the floating debug box, rduplieduce cate debug emissions, and refine dictionary-ready UI text.
...
class I18n:
    ...
    TEXT = {
        "zh_CN": {
            ...
            "ui_status_connected": "FaintOCR 3.7.1 已连接 OCR 控制器",
            ...
            "log_cleanup": "FaintOCR 3.7.1 资源清理完成",
        ...
        "en_US": {
            ...
            "ui_status_connected": "FaintOCR 3.7.1 connected to OCR controller",
            ...
            "log_cleanup": "FaintOCR 3.7.1 resources cleaned up",
...
class UserConfigManager:
    ...
    def save(cls, config):
        try:
            ...
            logger.warning(I18n.get("log_config_save_failed", exc))
...
class DictionaryWorker(QtCore.QThread):
    ...
    def run(self):
        ...
        try:
            ...
            logger.success(I18n.get("log_dict_success"))
...
class DetectorWorker(QtCore.QObject):
    ...
    def _on_timeout(self):
        try:
            ...
            if score > 1.0:
                logger.trace(I18n.get("debug_detect_poll", score, pixel_ratio, count_ratio))
                self._timer.stop()
...
class OCRController(QtCore.QObject):
    ...
    def _emit_dict_debug(self, message):
        logger.debug(I18n.get("debug_dict", message))
    ...
    def request_ocr(self, region, task_source="manual"):
        ...
        if self.ocr_engine_std is None or self.ocr_engine_light is None:
            self.ocr_result_signal.emit(False, I18n.get("err_ocr_engine_uninitialized"), "", False, task_source)
            return False
    ...
    def _on_worker_finished(self, handle_game, handle_source_lang, handle_target_lang, process_type, target_mask):
        ...
        if remote_updated_at:
            self._emit_dict_debug(
                I18n.get("log_dict_remote_result", I18n.game_name(handle_game), self._format_timestamp(remote_updated_at))
            )
        else:
            self._emit_dict_debug(I18n.get("log_dict_remote_unavailable", I18n.game_name(handle_game)))
...
class FaintOCRApplication(QtCore.QObject):
    ...
    def handle_dict_state(self, state, data_list, current_level, target_level):
        ...
        if state in {"MISSING", "STALE", "MISSING_STALE"}:
