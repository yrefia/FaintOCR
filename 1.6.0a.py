...
class OCRWorker(QtCore.QRunnable):
    ...
    def run(self):
        try:
            ...
            match_cost = t_match_end - t_match_start

            # 3. 构造界面所需的数据
            # 格式化耗时信息，保留3位小数
            perf_info = f"OCR+截图: {ocr_cost:.3f}s | 匹配: {match_cost:.3f}s"
...
class TextMatcher:
    ...
    def __init__(self, text_map, cache_size=3):
        ...
        self.candidates_t3 = []  # 前 3 个 Token 的前缀
    ...
    def _match_full_text(self, ocr_text, similarity_threshold=60):
        """完全匹配模式 (逻辑保持不变，但使用 candidates_full)"""
        if not self.text_map or not ocr_text.strip():
            return None, None, 0

        def get_val_str(key):
            val = self.text_map[key]
            if isinstance(val, list):
                return " / ".join(val)
            return str(val)

        if ocr_text in self.text_map:
            return ocr_text, get_val_str(ocr_text), 100
        ...
        if high_confidence_results:
            matched_keys = " | ".join([r[0] for r in high_confidence_results])
            matched_values = " | ".join([get_val_str(r[0]) for r in high_confidence_results])
            return matched_keys, matched_values, high_confidence_results[0][1]

        best_match, score, _ = results[0]
        if score >= similarity_threshold:
            return best_match, get_val_str(best_match), score

        return None, None, 0

    def _match_prefix_text(self, ocr_text, similarity_threshold=60):
        """
        [修改] 模式2：基于 Token 数量的智能前缀匹配
        """
        if not self.text_map or not ocr_text.strip():
            return None, None, 0
        ...
        # 2. 根据 Token 数量选择对应的候选列表
        # 逻辑：OCR 识别到了几个词，我们就去字典里找前几个词长得像的 key
        if token_count <= 3:
            target_candidates = self.candidates_t3
        elif token_count <= 5:
            ...
            # 如果识别到的文本很长（超过21个Token），则直接在完整列表中搜索
        ...
        if high_confidence_results:
            ...
            matched_values_str = " | ".join([get_val_str(k) for k in original_keys])
            return matched_keys_str, matched_values_str, high_confidence_results[0][1]

        best_match_str, score, idx = results[0]
        if score >= similarity_threshold:
            best_key = self.keys_list[idx]
            return best_key, get_val_str(best_key), score
...
class FloatingOCR(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        title_lbl = QtWidgets.QLabel("OCR 工具 (Pro)")
        ...
        """)
        self.download_progress.hide()  # 默认隐藏
